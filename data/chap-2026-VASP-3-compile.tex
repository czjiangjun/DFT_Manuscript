...
%编译安装。与许多一键安装的普通软件不同，
一般地讲，在\textrm{Linux}环境下，大型的科学计算软件的编译和维护，主要依赖\textrm{make}命令工具。因为科学计算软件常有大量源代码文件，用户不大可能逐个对源代码文件进行编译和链接。\textrm{make}命令工具，通过执行安装文件\textrm{makefile}指定的全部指令、实现软件的编译、链接的自动化构建，提高软件开发的效率。科学计算的过程常常会涉及大规模的矩阵运算、快速傅里叶变换\textrm{(Fast Fourier Transformation, FFT)}和并行通信，计算任务的效率也高度依赖编译器优化、数学库的算法实现以及并行通信库的性能。因此，科学计算软件的编译，根源于其对底层计算资源的深度理解和掌握。同样，\textrm{VASP}的编译，主要就是明确安装文件\textrm{makefile}中编译器版本、数学库、\textrm{MPI (Message Passing Interface)}并行库以及可选的\textrm{GPU}加速支持等变量的设定，以及涉及的编译、链接命令的参数的意义。%任何一个环节的疏漏都可能导致编译失败或运行时性能低下。\textrm{VASP}的编译也不例外。
理解\textrm{VASP}编译的每一个环节，不仅是成功安装\textrm{VASP}的前提，也是后续计算能够高效运行的基础。

%本文将从零开始，
这里介绍\textrm{VASP}编译的完整流程，涵盖编译器选择、重要数学库(包括\textrm{BLAS}、\textrm{LAPACK}、\textrm{ScaLAPACK}、\textrm{FFTW})的配置、\textrm{MPI}库的选型与设置\textrm{GPU}加速编译(\textrm{OpenACC}与\textrm{CUDA})。重点是安装文件\textrm{makefile.include}的必要说明。%以及常见问题的诊断与解决。无论你是初次接触VASP的新手，还是希望优化现有编译配置的老手，本文都将提供详实的参考。
...
\end{itemize}
用户可以根据自己的硬件配置，选择符合要求的一款编译器安装即可。在高性能计算集群中，相关的编译器一般都是安装好的，用户只需向集群运维和管理员确认编译器环境加载方案，遵照运行即可。
...
\begin{itemize}
	...
	\item \textrm{LAPACK}:~完成高级线性代数运算，如矩阵的分解或矩阵特征值求解等。
	\item \textrm{ScaLAPACK}:~完成分布式内存、并行环境下各类线性代数运算。
	\item \textrm{FFTW}:~完成快速\textrm{Fourier}变换。
\end{itemize}
\textrm{VASP}计算中会有大量矩阵的构造、分解和矩阵对角化，物理量也会有实空间和倒空间的多次变换，因此这些库函数是完成计算所必须的。一般地，如果用户使用\textrm{Intel oneAPI}套件，\textrm{BLAS}、\textrm{LAPACK}和\textrm{ScaLAPACK}和\textrm{FFTW}库均已包含在\textrm{MKL}库中，无须单独安装。如果使用\textrm{NVIDIA HPC-SDK}，\textrm{BLAS}、\textrm{LAPACK}和\textrm{ScaLAPACK}同样随\textrm{SDK}一同提供，唯一需要自行安装的数值库是\textrm{FFTW}。
...
\end{itemize}
在高性能计算集群环境下，\textrm{MPI}属于标配，用户只需向集群运维和管理员确认编译器环境加载方案，遵照运行即可。
%### 1.4 可选依赖库
%\section{可选的依赖库}
...
用户自定义和修改\textrm{makefile.include}的主要步骤
\begin{enumerate}
...
此处用户设定\textrm{FC}和\textrm{FCL}指向的编译器为\textrm{mpiifort}，它应该是已加载过的环境变量(通过命令\texttt{which mpiifort}可以获取)，否则必须指定编译器的绝对路径。
	\item 设置各类库函数路径:~包括\textrm{MKL\_ROOT}、\textrm{FFTW\_ROOT}、\textrm{NVROOT}等并指定必要的库函数名:\\
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=25-25]{Figures/makefile.include.linux_intel_omp} %为保险:~选用文件名绝对路径
}}
此处用户设定\textrm{MKLROOT}(或\textrm{MKL\_PATH})的路径，必须是\textrm{Intel}的\textrm{MKL}库的路径(通过命令\texttt{echo \$MKLROOT}可以获取)。%比较老的\textrm{VASP}版本
%   - 确认`FFTW_ROOT`或`FFTW`路径正确
%   - 确认`NVROOT`正确（NVIDIA HPC-SDK）
	\item 选择预编译选项:~根据需要启用或禁用功能
	\item 调整优化级别:~根据硬件和需求调整\textrm{OFLAG}
\end{enumerate}
原则上，如果用户的环境变量已经正确加载，只要在\textrm{makefile.include}文件中修改这几处，即可完成\textrm{VASP}软件的正确编译。编译命令为\texttt{make all}，产生的\textrm{VASP}可执行版本为\textrm{vasp\_std}、\textrm{vasp\_gma}和\textrm{vasp\_nlr}。

\section{常见\rm{makefile.include}配置}
%
