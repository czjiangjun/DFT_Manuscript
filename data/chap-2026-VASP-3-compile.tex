...
\begin{itemize}
	\item - **Intel oneAPI**（含ifort/ifx编译器）
	\item NVIDIA HPC-SDK**（含nvfortran编译器）——**特别地，对于GPU版本的OpenACC编译，必须使用NVIDIA HPC-SDK（版本≥21.2）**
	\item GCC/GFortran**（版本≥7.X，但仅从VASP 6.2.0开始支持）
	\item AMD AOCC**（AMD优化编译器，适用于AMD EPYC处理器）
\end{itemize}<++>
