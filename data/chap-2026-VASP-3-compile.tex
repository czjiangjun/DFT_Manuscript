...
\textrm{VASP}的计算会涉及大规模的矩阵运算、快速傅里叶变换\textrm{(Fast Fourier Transformation, FFT)}和并行通信，计算任务的效率也高度依赖编译器优化、数学库的算法实现以及并行通信库的性能。因此，\textrm{VASP}的编译根源于其对底层计算资源的深度依赖。理解\textrm{VASP}编译的每一个环节，不仅是成功安装\textrm{VASP}的前提，也是后续计算能够高效运行的基础。
...
\textrm{VASP}支持分布式内存并行计算，通过\textrm{Message Passing Interface (MPI)}实现并行计算的信息同步与通信，\textrm{VASP}推荐的\textrm{MPI}库包括:
\begin{itemize}
...
\subsection{\rm{makefile.include}详解}
\textrm{makefile.include}是\textrm{VASP}编译中的核心配置文件，它确定\textrm{make}工具\footnote{\textrm{make}是\textrm{Linux}操作系统下的一个构建工具，通过执行\textrm{make}命令，解释\textrm{makefile}文件中指定的指令，完成自动化编译和构建软件项目。即\textrm{make}可用于开发人员管理和维护复杂的软件项目，自动化编译过程并生成可执行文件。}所使用的编译器、编译选项、链接的库函数。%VASP官方明确表示：“从头编写makefile.include文件并不容易，因此建议从与系统最接近的模板文件开始”。

%### 6.1 模板文件的位置与选择
\textrm{VASP}源代码的\textrm{arch/}目录下，提供了大量的模板文件，一般包括：
\begin{itemize}
...
\item 编译器及编译选项
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=13-36]{Figures/makefile.include.linux_intel_omp} %为保险:~选用文件名绝对路径
}}
...
\textrm{BLAS/BLACS} &涉及向量运算、矩阵乘法的基本线性代数运算库 (\textrm{BLAVS}支持并行计算)\\
\textrm{LAPACK/SCALAPACK} &涉及矩阵对角化和本征值求解的线性代数运算库(\textrm{SCALPACK}支持并行计算)\\
\textrm{OBJECTS}  &面向\textrm{FFTW}所需要的各类数学库\\
...
用户自定义\textrm{makefile.include}的主要步骤
\begin{enumerate}
	\item 选择模板:~从\textrm{arch/}目录复制最接近的模板
%   ```bash
%   cp arch/makefile.include.your_choice ./makefile.include
%   ```

	\item  指定编译器的正确路径
	%：确认`FC`和`FCL`指向正确的编译器

	\item 设置各类库函数路径:~包括\textrm{MKL\_ROOT}、\textrm{FFTW\_ROOT}、\textrm{NVROOT}等并指定必要的库函数名
...
\subsection{常见\rm{makefile.include}配置}
