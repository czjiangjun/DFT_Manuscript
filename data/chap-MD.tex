...
对势是最简单的一种相互作用势，它只考虑两个原子之间的作用，不再考虑其他的相互作用。一个典型的例子是\textrm{Lennard-Jones (L-J)}势\cite{PRS106-463_1924}，$U_{\mathrm{LJ}}(r)$，用原子间距离$\mathbf{r}$表示，\textrm{L-J}势的表达式有两个参数:~
\begin{equation}
...
\subsection{初值预置}\label{MD:Initial}
...
\subsection{初始化}\label{MD-speed-Initial}
...
\subsection{分子动力学中的能量计算}\label{sec:MD-Etot}
