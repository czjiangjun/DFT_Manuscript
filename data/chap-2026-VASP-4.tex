\chapter{\rm{VASP}教程与算例}
\textrm{VASP}的官方网站提供了系统而完整的示范教程%\url{https://vasp.at/wiki/index.php/Category:Tutorials}
\url{https://vasp.at/tutorials/latest/}，这个教程在一直在维护和更新。笔者以为这是迄今为止最完整的\textrm{VASP}教程，除了对各算例有比较详细的输入、输出内容的说明，包括编译、安装及执行在内，都有详细的指导，有兴趣的\textrm{VASP}用户，特别是初学者，建议遵照教程尝试练习。本章也将参照教程的算例，对\textrm{VASP}具体计算中的内容作必要说明。另外，\textrm{June Gunn Lee}编著的\textrm{Computational Materials Science:~an introduction}\cite{J.-G._Lee}一书也有\textrm{VASP}算例的相关章节，可资参阅。
\section{基本计算}
\subsection{原子基态能量的计算}\label{Sec:atom-O}
真空中的孤立原子的基态能量是\textrm{VASP}中最简单的算例。本算例提供了计算单个\ch{O}原子能量的示范。初学者可以掌握典型的\textrm{VASP}的主体流程\footnote{在所有计算之前，请确认\textrm{VASP}软件已经正确安装。}，了解体系基态能量最小化的基本算法，并熟悉基本的输入/输出文件的内容。

\subsubsection{孤立原子/分子计算}\label{Sec:atom-O-1}
\textrm{VASP}完成的是周期体系的\textrm{DFT}计算，所以对于孤立的\ch{O}原子，是将该原子置于一个足够大的晶胞内，然后根据周期边界条件计算孤立原子的相关能量。\textrm{DFT}计算的基本变量是体系的电荷密度，其计算过程可以概括为：
\begin{enumerate}
	\item 根据体系的初始电荷密度，确定体系的\textrm{Kohn-Sham}方程的\textrm{Hamiltonian}；
	\item 对角化\textrm{Hamiltonian}矩阵，获得体系的本征态波函数和本征值，并计算体系各项能量；
	\item 根据得到的体系本征态波函数，更新体系的电荷密度；
	\item 重复步骤\textrm{1.-3.}直到体系总能量收敛。
\end{enumerate}

表示周期体系中的\ch{O}原子的\textrm{POSCAR}中，晶胞参数设置为足够大(8\AA)，确保近邻晶格的\ch{O}原子间不会存在显著的相互作用:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-8]{Figures/VASP_tutorials-1-1_POSCAR} %为保险:~选用文件名绝对路径
}}

如前所述，\textrm{INCAR}文件包含的是各类计算控制参数。为了减轻用户设置控制参数的复杂手续，\textrm{VASP}程序对每个没有列入\textrm{INCAR}文件的控制参数都设为其默认值。
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-8]{Figures/VASP_tutorials-1-1_INCAR} %为保险:~选用文件名绝对路径
}}
初学者如有兴趣，可以留意检索\textrm{VASP}手册的相关章节，了解控制参数\texttt{SYSTEM}和\texttt{ISMEAR}=0的含义。

对于原子体系，\textrm{KPOINTS}文件设置为:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-5]{Figures/VASP_tutorials-1-1_KPOINTS}%为保险:~选用文件名绝对路径
}}
对于孤立原子或分子计算，一个$\vec k$点已经足够。原则上，\textrm{KPOINTS}文件中包含的波矢$\vec k$越多，计算得到的原子间相互作用越精确。但因为本算例中，由于晶胞参数足够大，孤立\ch{O}原子间相互作用作用可以忽略，没有必要徒劳使用更多的$\vec k$点。

以下给出计算所需的\ch{O}元素的\textrm{POTCAR}文件的前几行：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-12]{Figures/VASP_tutorials-1-1_POTCAR}%为保险:~选用文件名绝对路径
}}
需要注意的是，用户不要随便改动\textrm{POTCAR}的内容。\textrm{VASP}官网明确指出，普通用户没必要了解\textrm{POTCAR}的详细内容，只需要注意\textrm{POTCAR}文件第4行\ch{O}元素的价电子构象(\textrm{O: s2p4})和第12行的参数\textt{ZVAL}=6(表示\ch{O}元素的6个价电子)，两者是保持一致的。

运行\textrm{VASP}程序，完成计算后，屏幕的标准输出如下所示：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-24]{Figures/VASP_tutorials-1-1_stdout}%为保险:~选用文件名绝对路径
}}
标准输出中，在\textrm{entering main loop}前面，是计算资源配置和初始化；在这一行之后输出的内容与\textrm{OSZICAR}文件内容一致，每一列的含义，可参阅\ref{subsectr:OSZICAR}的相关部分内容，兹不赘述。

根据\textrm{VASP}的程序设计，最初的电子密度源于\textrm{POTCAR}；为了迭代对角化的稳定性，在最初的四次电子迭代步，电荷密度保持不变，之后每一步的电荷密度才开始更新，所以最初四次电子迭代，\textrm{rms(c)}没有数据输出。默认的电子步收敛的标准是$\mathrm{dE}<10^{-4}$和$\mathrm{d~eps}<10^{-4}$。

更完整的输出信息在\textrm{OUTCAR}文件中，当计算对象达到收敛后，\textrm{OUTCAR}中体系能量本征值。有必要指出的是，由于本次采用默认的低精度计算，读者自行完成的计算结果，很可能因为具体计算硬件及相关配置的影响，和这里给出的结果有微小的数值差异:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-15]{Figures/VASP_tutorials-1-1_OUTCAR}%为保险:~选用文件名绝对路径
}}
\textrm{E-fermi}的值是\textrm{-8.8433}，单位是\textrm{eV}。固体物理中，习惯上先将\textrm{Fermi}能级置零。然后再讨论体系的\textrm{Kohn-Sham}方程的本征态(\textrm{band energies})。而体系的占据态(\texttt{occupation})的能量都应该非正值。

\textrm{DFT}计算结果与实际的孤立原子的物理态的关系:\\
\ch{O}([\ch{He}]$2s^22p^4$)共有6个价电子，两个$2s$电子和四个$2p$电子。根据上述讨论可知，$2s$电子的能量本征(相比于\textrm{Fermi}能级)为：
\begin{displaymath}
	-23.8439\mathrm{eV}+8.8431\mathrm{eV}=-15.0008\mathrm{eV}
\end{displaymath}
$2s$电子比$2p$电子更局域，在\textrm{DFT}理论中，4个简并的$2p$电子平均占据3个$2p$轨道，因此这里简并的能带(轨道)2,3,4的占据数为1.3333。

体系最终的自由能为：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=16-20]{Figures/VASP_tutorials-1-1_OUTCAR}%为保险:~选用文件名绝对路径
}}
而体系的熵的贡献是
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=22-22]{Figures/VASP_tutorials-1-1_OUTCAR}%为保险:~选用文件名绝对路径
}}
由于\textrm{DFT}计算得到的简并$2p$轨道，会引入非物理的熵(\mathrm{S})的贡献。参数\texttt{SIGMA}是计算引入的\textrm{Gaussian}展宽，它可以视为认为引入的电子温度\textrm{T}，小的\texttt{SIGMA}会降低熵的贡献，但可能会引起\textrm{Fermi}积分的变慢。这部分非物理的贡献就是自由能计算的熵的来源。

需要指出的是，计算得到的孤立\ch{O}原子总能\textrm{energy without entropy}接近于0,严格地讲，如果选择的晶格足够大，并且计算精度足够高，该值应该严格为0，因为用于计算的赝势是孤立非极化的原子生成的。原则上说，用于计算的赝势的构象(体系能量计算的参照点)是任意的，但对于非孤立体系(比如小一点的晶胞中有若干个原子)，计算得到的能量绝对值意义不大，只有相对能量(即本征态能量相对\textrm{Fermi}能，总能相对参照点)才是有意义的。

一般地，如果在计算完成后，重启计算，\textrm{VASP}将从\textrm{WAVECAR}读入波函数，计算将很快收敛。如果删除\textrm{WAVECAR}，体系将重新开始完整计算。

\subsubsection{考虑自旋极化的原子基态能量}\label{Sec:atom-O-2}
对于孤立\ch{O}原子，考虑自旋极化计算后可以得到\ch{O}原子的自旋磁矩，根据自旋密度泛函理论\textrm{(Spin Density Functional Theory, SDFT)}，\ch{O}原子中自旋向上\textrm{(spin-up)}和自旋向下\textrm{(spin-dn)}态和电荷密度将解耦，两种自旋允许具备不同的本征态。考虑自旋的体系总能与非自旋体系的能量差，反映了两种不同自旋态的相互作用大小。

自旋极化计算的\textrm{POTCAR}、\textrm{POSCAR}和\textrm{KPOINTS}与\ref{Sec:atom-O-1}中相同，\textrm{INCAR}参数设置如下：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-3]{Figures/VASP_tutorials-1-2_INCAR}%为保险:~选用文件名绝对路径
}}
这里为了分别考虑不同自旋的贡献，增加参数\texttt{ISPIN}=2。

使用这四个输入文件，开启新的计算。这里顺便说明一下\textrm{vasp\_std}和{vasp\_gam}两种\textrm{VASP}计算的区别：当\textrm{KPOINTS}文件中的$\vec k$点仅有$\Gamma$点(即(0, 0, 0)点)时，可以采用\textrm{vasp\_gam}替代\textrm{vasp\_std}命令，这样做，计算过程中的某些数组将会采用实数型，而不必是复数型。因此采用\textrm{vasp\_gam}计算，可以节约内存开销和计算成本。

屏幕显示的标准输出，将会出现示警信息(\textrm{WARRING!!!}):
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-50]{Figures/VASP_tutorials-1-2_stdout}%为保险:~选用文件名绝对路径
}}
这里出现示警信息是因为在\textrm{INCAR}文件中设置了参数\texttt{ISPIN}=2，但并未指定参数\textrm{MAGMOM}的初始值。\textrm{VASP}计算中在标准输出和\textrm{OUTCAR}出现示警信息，仅是一种提示，并不意味着计算一定会出错(错误信息的提示为\textrm{ERROR!!!})，提醒用户留意确认当前计算是否是预期想执行的，有必要检查控制参数；示警信息有助于用户尽早避免一些常见的误操作，从而提升\textrm{VASP}计算的效能。

类似地，在输出文件的电子步收敛后，可以看到自旋极化计算的自旋本征态能量和占据数，可以与非自旋极化计算的结果对照：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-25]{Figures/VASP_tutorials-1-2_OUTCAR}%为保险:~选用文件名绝对路径
}}
显然，在\textrm{SDFT}计算中，不同自旋电子的本征态能量是不同的，根据\textrm{Hund}规则，4个$2p$电子中3个优先占据\textrm{spin-up}\footnote{\textrm{VASP}计算结果只是通过自旋态1\textrm{spin component 1}和自旋态2\textrm{spin component 2}来标示不同自旋，这里根据习惯，称前者为\textrm{spin-up}。}轨道(\textrm{occupation 1.00})；剩下1个电子进入相反自旋\textrm{spin-dn}轨道，如前所说，\textrm{spin-dn}轨道电子的分数占据(\textrm{occupation 0.3333})，源于\textrm{DFT}理论中基本变量是体系的电子密度，而非波函数。

孤立\ch{O}原子的自旋磁矩，可以通过两个不同自旋的磁矩的抵扣后的净磁矩计算得到，换言之，如选择体系的磁化方向为$z$轴方向，则有
\begin{displaymath}
	\langle\mu_{s,z}\rangle=\mathbf{g}_s\langle\dfrac12\sigma_z\rangle\mu_B
\end{displaymath}
这里$\mathbf{g}_s\approx2$是自旋的$g$因子，$\sigma_z$表示\textrm{Pauli}矩阵，$\mu_B$为\textrm{Bohr}磁子。不难看出，体系达到收敛时，\textrm{spin-up}的电子数多出2个电子，因此$\langle\mu_{s,z}\rangle=2\mu_B$。所以最后屏幕显示的标准输出有体系磁矩\textrm{mag}=1.999，与$\langle\mu_{s,z}\rangle$的数值一致。

\subsubsection{点群对称性的影响}\label{Sec:atom-O-3}
\textrm{VASP}计算中，为了节约计算资源，软件会主动检查并判断体系所属的点群对称性。利用对称性原理，减少计算开销，对于具有对称性的复杂体系，这样做毫无疑问是很有必要的。但有必须说明的是，\textrm{DFT}计算和描述的体系基态，并没有对称性的要求与约束。如果用户计算中要避免体系对称性的约束(或者人为地使用不同于对称性结构)，建议手动关闭软件的对称性检查功能。\textrm{INCAR}文件中，对称性检查与参数控制为\texttt{ISYM}。

为了说明点群对称性对计算的影响，在\ref{Sec:atom-O-2}中，孤立\ch{O}原子的晶格具有$\mathrm{O}_h$点群立方对称性，现将对称性降为$\mathrm{D}_{2h}$正交对称性，其\textrm{POSCAR}文件修改为：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-8]{Figures/VASP_tutorials-1-3_POSCAR}%为保险:~选用文件名绝对路径
}}
\textrm{POTCAR}、\textrm{INCAR}文件和\textrm{KPOINTS}文件都保持不变，

提交\textrm{vasp\_gam}计算结束后，检索输出文件\textrm{OUTCAR}：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-31]{Figures/VASP_tutorials-1-3_OUTCAR}%为保险:~选用文件名绝对路径
}}
可以看到，体系属于正交对称性\textrm{point symmetry D\_2h}，即体系所属的点群对称性为$\mathrm{D}_{2h}$，而此前\ref{Sec:atom-O-2}中，体系属于立方对称性\textrm{point symmetry O\_h}，即$\mathrm{O}_h$。

对比正交点群对称性条件下的体系总能\textrm{(energy without entropy)}：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-3_OUTCAR}%为保险:~选用文件名绝对路径
}}
远低于未考虑自旋极化的孤立\ch{O}原子参考点能量(见\ref{Sec:atom-O-1})。不难看出，考虑自旋极化后，\ch{O}原子的$2p$电子占据方式更接近真实的基态构象，因此能量大大降低，降低对称性也会引起基态能量的少量降低，这称为对称性破缺\textrm{(Symmetry Breaking)}。\textrm{VASP}计算默认使用的\textrm{GGA}泛函的贡献，对绝大部分原子，都是在打破对称性条件下得到的计算完成的。

最后，在\textrm{INCAR}文件中增加参数\texttt{SIGMA}=0.01，并重新完成计算，在\textrm{OUTCAR}文件中，体系的总能变成：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=35-35]{Figures/VASP_tutorials-1-3_OUTCAR}%为保险:~选用文件名绝对路径
}}
显然，仅仅改变\textrm{Gaussian}展宽参数，体系能量也将有较大改变。这说明，如果两个计算体系能量要对比，其\textrm{Gaussian}展宽参数尽量应尽量保持一致。

\subsubsection{分子中的键长}\label{Sec:atom-O-4}
结构弛豫是\textrm{VASP}中最常见的计算类型之一。由于原子核的质量远大于电子质量，根据\textrm{Born-Oppenheimer}近似，可将两者的运动分别处理，考察电子运动的时候可以认为原子核的位置是固定不动的；而考虑原子核/离子运动时，可以认为它们是在电子运动构成的能量面上运动，这个能量面一般称为势能面\textrm{(Potential Surface)}。结构弛豫就是在势能面上找到体系中各原子所受的受力和应力都为零的位置。一般第一原理计算软件完成结构弛豫的步骤为:
\begin{enumerate}
	\item 在体系的初始结构，采用\textrm{DFT}方法得到体系的基态能量；
	\item 根据\textrm{Hellmann-Feyman}定理，利用电子\textrm{Hamiltonian}的梯度，可以计算离子所受力和应力
		\begin{displaymath}
			\nabla_nE=\dfrac{\lanele\Psi_G|\nabla_n\mathrm{H}|\Psi_G\rangle}{\langle\Psi_G|\Psi_G\rangle}
		\end{displaymath}
\end{enumerate}
\item 根据优化算法，离子依照受力，朝有利于当前能量优化方向移动；
\item 重复步骤1.-3. 直到体系总能达到预设的收敛标准。
\end{enumerate}

\ch{O2}分子弛豫的初始结构\textrm{POSCAR}：
在大的晶胞中，两个\ch{O}原子的初始距离是1.22\AA。
\textrm{KPOINTS}和\textrm{POTCAR}不变。
输入控制参数\textrm{INCAR}
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-4_INCAR}%为保险:~选用文件名绝对路径
}}
计算中考虑原子自旋，参数\texttt{NSW}指定结构弛豫中离子步移动的上限，\texttt{IBRION}=2表示采用共轭梯度\textrm{(Conjugate Gradient, CG)}算法完成结构弛豫所需要的能量优化。关于\textrm{CG}算法参见\textcolor{red}{有关章节}。一般地，当弛豫对象的自由度较低($\leqslant4$)并且偏离基态较小，\textrm{CG}算法非常高效；否则，建议用户通过参数\texttt{IBRION}选择不同的结构弛豫算法来完成能量搜索。

结构弛豫计算完成后，用户可以通过标准输出\textrm{stdout}或\textrm{OSZICAR}文件，统计完成结构弛豫使用的电子步和离子步的数目，并在最后检查\textrm{CONTCAR}中的原子结构，与\textrm{POSCAR}中的数据对照，检查并确定结构弛豫对晶胞数据和原子位置的改变。也可以通过\textrm{py4vasp}查看相关数据。
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-4_CONTCAR}%为保险:~选用文件名绝对路径
}}
\begin{lstlisting}[language=Python, basicstyle=\ttfamily, keywordstyle=\color{blue}, stringstyle=\color{red!70!black}]
import py4vasp
my_calc = py4vasp.Calculation.from_path("./e04_02-bond")

my_calc.structure.print()
\end{lstlisting}

类似地，如果应用\textrm{CG}算法完成\ch{CO}的结构弛豫，\textrm{VASP}建议，对于原子间间距(键长)较短的双原子分子，如\ch{O2},\ch{CO},\ch{N2},\ch{F2},\ch{P2},\ch{S2},\ch{Cl2}等，选择较为硬的原子数据集。

结构文件\textrm{POSCAR}的内容为:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-5_POSCAR}%为保险:~选用文件名绝对路径
}}
计算控制文件\textrm{INCAR}的内容为:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-5_INCAR}%为保险:~选用文件名绝对路径
}}
对于复杂体系的\textrm{POTCAR}文件的组织命令如下(注意确保\textrm{POSCAR}中原子顺序一致)
\begin{lstlisting}[language=bash, basicstyle=\ttfamily\small, keywordstyle=\color{blue}, commentstyle=\color{green!60!black}, stringstyle=\color{red!70!black}, frame=single]
cd $TUTORIALS/molecules/e05_*
    cat pot/C_h/POTCAR pot/O_h/POTCAR > POTCAR
\end{lstlisting}
较硬的原子数据集会需要更多的计算资源开销，当计算精度不高时，可以采用标准的原子数据集而非较硬的。

在完成计算之后，学习修改参数后重启计算：设置参数\texttt{POTIM}=0.2(默认为\texttt{POTIM}=0.5)，重启计算
\begin{lstlisting}[language=bash, basicstyle=\ttfamily\small, keywordstyle=\color{blue}, commentstyle=\color{green!60!black}, stringstyle=\color{red!70!black}, frame=single]
cd $TUTORIALS/molecules/e05_*
    rm WAVECAR
...
\end{lstlisting}
随后，可以用\textrm{Python}模块\textrm{py4vasp}查看可视化的分子结构。
\begin{lstlisting}[language=bash, basicstyle=\ttfamily\small, keywordstyle=\color{blue}, commentstyle=\color{green!60!black}, stringstyle=\color{red!70!black}, frame=single]
# 这是一个 Bash 注释
$ pip install py4vasp
$ my_calc = py4vasp.Calculation.from_path("./e04_02-bond")
...
\subsubsection{分子振动频率的计算}\label{Sec:atom-CO-2}
实际体系中任何分子都存在振动，很多化学键都存在特征的振动频率(10-100\textrm{Thz}不等)。在\textrm{VASP}计算中，分子的振动频率与$\Gamma$点的声子振动频率相似(\textcolor{red}{关于声子频率的计算参见理论进阶部分相关章节。})原则上，会涉及计算\textrm{Born-Oppeneheimer}近似下势能面$E(\mathbf{R})$的一阶和二阶导数。该势能面确定了离子的运动方式。
\begin{displaymath}
	\bigg(\sum_{\mu}\dfrac{\hbar^2}{2M_{\mu}}\nabla_{\mathbf{R}_{\mu}}^2+E(\mathbf{R})\bigg)\zeta(\mathbf{R})=\varepsilon\zeta(\mathbf{R})
\end{displaymath}
氏中$\mathbf{R}_{\mu}$是第$\mu$个离子的坐标位置，离子质量为$M_\{\mu}$，$\zeta{\mathbf{R}}$和$\varepsilon$分别是离子体系的本征态和本征值。在\textrm{Born-Oppeneheimer}近似条件下，$E(\mathbf{R})$是体系中各离子分别位于$\mathbf{R}=\{\mathbf{R}_{\mu}\}$时，电子体系的基态能量。

求解离子体系的振动频率$\omega$就是求解
\begin{displaymath}
	\det\bigg|\dfrac{1}{\sqrt{M_{\mu}M_{nu}}\dfrac{\partal^2E(\mathbf{R})}{\partial R_{\mu i}\partial R_{\nu j}}-\omega^2\bigg|=0
\end{displaymath}
其中$\dfrac{\partial^2E(\mathbf{R})}{\partial R_{\mu i}\partial R_{\nu j}}$称为对应离子位置变量$\mu$和$\nv$的\textrm{Hessian}矩阵，$i,j=x,y,z$、

计算中的初始结构文件\textrm{POSCAR}
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-5_POSCAR}%为保险:~选用文件名绝对路径
}}
控制参数文件\textrm{INCAR}为:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-6]{Figures/VASP_tutorials-1-6_INCAR}%为保险:~选用文件名绝对路径
}}
其中参数\texttt{IBRION}=5,采用有限差分方法计算\ch{CO}分子的振动频率，因此设置参数\texttt{NFREE}和\textrm{POTIM}。\textrm{KPOINTS}和\textrm{POTCAR}文件保持不变。
计算结束后，在输出文件\textrm{OUTCAR}的最后，可以看到分子\textrm{CO}的正则振动模信息：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=33-33]{Figures/VASP_tutorials-1-6_INCAR}%为保险:~选用文件名绝对路径
}}
根据文献\cite{JCE_1996}，实验观测得到\ch{CO}的振动频率为$\textrm{2143cm}^{-1}$，即64.25\textrm{THz}，而计算值为63.887522\textrm{THz}，两者非常接近。

\subsubsection{分波态密度的计算}\label{Sec:atom-CO-5}
\textrm{DOS}描述的是在单位能量范围内，体系具有电子态的数目。因此可以
\begin{displaymath}
	N=\int_{-\inty}^{\inty}f(epsilon)D(epsilon)
\end{displaymath}
这里$N$是总的态的数目，$f(\epsilon)$是\textrm{Fermi}分布函数，$D(\epsilon)$表示\textrm{DOS}随能量$\epsilon$变化的函数。在有限温度下的相互作用电子体系，该函数也称为谱函数\textrm{(spectral function)}，常用于实验中的光电子谱\textrm{(Phote Electron Spectroscopy, PES)}的评估。

一般地，可以将态密度细分为分波态密度\textrm{(partial DOS, PDOS)}，局域态密度\textrm{(local DOS, LDOS)}和总态密度\textrm{(totla DOS, TDOS)}。其中\textrm{PDOS}表示指定原子轨道在\textrm{DOS}中的贡献。在这个算例中，总态密度\textrm{TDOS}来自全部\ch{CO}分子的贡献，据此可以计算\ch{O}的$2p$轨道的分波态密度\textrm{(PDOS)}。对于周期体系，由于电子波函数还是波矢$\vec k$的函数，因此计算态密度要考虑$\vec k$空间的贡献，局域态密度\textrm{(LDOS)}就是对$\vec k$空间积分得到的。

计算中的结构文件\textrm{POSCAR}为：
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-10]{Figures/VASP_tutorials-1-7_POSCAR}%为保险:~选用文件名绝对路径
}}
注意，如果只完成\textrm{DFT}静态计算，在\textrm{POSCAR}文件中引入\textrm{selective degree of freedom}不会有任何影响。
计算控制文件\textrm{INCAR}为:
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-3]{Figures/VASP_tutorials-1-7_INCAR}%为保险:~选用文件名绝对路径
}}
这里参数\texttt{LORBIT}=11，\textrm{VASP}将计算得到的分波投影函数输出到文件\textrm{PROCAR}。
使用\textrm{Python}模块\textrm{py4vasp}可以帮助用户将\textrm{DOS}作图，也可以引入字符串\textrm{"p"}完成分波态密度作图。
\begin{lstlisting}[language=Python, basicstyle=\ttfamily, keywordstyle=\color{blue}, stringstyle=\color{red!70!black}]
import py4vasp
...
# my_calc.dos.plot("p")
\end{lstlisting}
\subsubsection{分子计算算例:\rm{H$_3$O}}{Sec:atom-H2O}
除了\textrm{CG}方法，结构弛豫常用的方案还有残差最小化方法\textrm{(the residual minimization method, RMM-DIIS)}方法，一般称为广义最小残差法\textrm{(generalized minimal residual method, GMRES)}，是一种准\textrm{Newton}算法\textrm{(quasi-Newton algorithm)}。
\textcolor{red}{按照迭代子空间直接反演\textrm{(Direct Inversion in the Iterative Subspace,DIIS)}最小化方法的原则，只要经过若干次迭代，就能获得体系的最小化，是一种直接求解的方法。在子空间得到最终结果。具体到\textrm{RMM}，这里最小化的是作用于原子的力而非体系的总能。对于一个作用于电子的连续的势函数$V(\vec r,\mathbf{R})$，它依赖于参数化的离子位置$\mathbf{R}=\{\mathbf{R}_{\mu}\}$，根据\textrm{Hellmann-Feynman}定理，原子的受力和\textrm{Hessian}矩阵的矩阵元可以表示为
\begin{displaymath}
	\begin{aligned}
		F_{\mu i}=\dfrac{\partial E}{\partial R_{\mu i}}=\int\dfrac{\partial V(\vec r;\mathbf{R})}{\partial R_{\mu i}}n(\vec r;\mathbf{R})\mathrm{d}\vec r\\
		H_{\mu i\nu j}=\dfrac{\partial^2E}{\partial R_{\mu i}\partial R_{\nu j}}=\int\dfrac{\partial^2V(\vec r;\mathbf{R})}{\partial R_{\mu i}\partial R_{\nu j}}n(\vec r;\mathbf{R})\mathrm{d}\vec  r+\int\dfrac{\partial V(\vec r;\mathbf{R})}{\partial R_{\mu i}}\dfrac{n(\vec r;\mathbf{R})}{\partial R_{\nu j}}\mathrm{d}\vec r
	\end{aligned}
\end{displaymath}
这里$\mu,\nu$表示离子位置，$i,j=x,y,z$，$n(\vec r;\mathbf{R})$是电子电荷密度。\textrm{RMM_DIIS}方案最小化的是力的参量，即$|\mathbf{F}|^2$，这里
\begin{displaymath}
	F_{\mu i}=\sum_{\nu j}H_{\mu i\nu j\partial R_{\nu j}}
\end{displaymath}
注意到\textrm{RMM-DIIS}方法会根据\textrm{Hessian}矩阵的特征值分布，把最小化问题拆分到不同的子空间来处理。因此该算法非常高效，当然该方法对初值的选择会更敏感，也会存在其它偶发的问题，因此用户要特别注意。对于离子临近稳定位置，且自由度$\geqtslant3$的体系，该方法是好的选择，而对于自由度$geqslant$体系，应考虑其余的优化或分子动力学计算方法。}

在\textrm{POSCAR}文件中，原子\ch{O}位于原点，\textrm{H-O}的键长约为$0.96\AA$，而\textrm{H-O-H}夹角为$105^{\circ}$，晶胞参数设置得足够大。确保\ch{H2O}是孤立分子，通过\texttt{Selective dynamic}参数，允许两个氢原子在$xy$平面内自由活动。这个离子中\textrm{POSCAR}第二行的\textrm{scaling parameter}是晶格参数和\textrm{Cartian}坐标表示的原子位置都需要乘的系数。

这里的\textrm{INCAR}中，参数\texttt{IBRION}=1，表明结构优化采用\textrm{RMM-DIIS}算法；\texttt{NFREE}=2，表示计算对象有两个独立的自由度；\texttt{EDIFFG}取负值表示离子步采用力收敛标准；\texttt{ENCUT}取到比默认值(\textrm{POTCAR}中的\textrm{ENMAX}的最大值)大约30\%。这样的参数取值也是\textrm{VASP}计算弛豫时，考虑到计算应力张量和压应力时的推荐设置。

本次计算，\ch{H}和\ch{O}选择标准原子数据集，而非硬核类型，只是因为作为示范计算，节约计算开销考虑。

运行\textrm{VASP}，可以在标准输出\textrm{stdout}或者\textrm{OSZICAR}中，看到\textrm{RMM-DIIS}算法的计算收敛，用户可以与\textrm{CG}算法对比。

通过\textrm{py4vasp}模块，可以观察结构弛豫对\textrm{H-O}的影响。对照\textrm{CONTCAR}数据，计算\textrm{H-O}键长和\textrm{H-O-H}的键角:
\begin{itemize}
	\item \textrm{H-O}键长:
		\begin{displaymath}
			\bigg[(0.09379929998266598)^2+(0.1208431793169455)^2\bigg]^{1/2}\times12\times0.52918\\
			=0.9714163904\AA
		\end{displaymath}
	\item \textrm{H-O-H}键角:
		\begin{displaymath}
			\arccos\dfrac{0.09379929982665981^2-0.12084317931694552^2}{0.09379929982665981^2+0.12084317931694552^2}\\
			=104.3622835581412\textdegree
		\end{displaymath}
\end{itemize}
除了计算，也可以在\textrm{py4vasp}模块的可视化环境下直接获取有关键长、键角数据:
\begin{itemize}
	\item 构建$2\tiems2\times2$超晶胞
	\item 右键点击第一个原子，再右键双击第二个原子，即可得到两个原子间的键长
	\item 如果再右键双击第三个原子，即可得到对应的原子形成的键角
\end{itemize}

\ch{H2O}计算中的能量截断与参数\texttt{PREC}

在计算控制文件\textrm{INCAR}中，存在控制参数\texttt{PREC}和\texttt{ENCUT}。参数\texttt{ENCUT}是能量截断收敛参数。

在\textrm{PAW}方法中，\textrm{Kohn-Sham}方程解出的波函数形式为
\begin{equation}
	|\Psi_{i\vec k\sigma}\rangle=|\tilde{\Psi}_{i\vec k\sigma}\rangle+\sum_{\eta}\bigg(|\phi_{\eta}\rangle-|\tilde\phi_{\eta}\rangle\bigg)\langle\tilde p_{\eta}|\tilde\Psi_{i\vec k\sigma}\rangle
	\label{eq:PAW-eq-1}
\end{equation}
波函数分解为离域的赝波函数$|\tilde{\Psi}_{i\vec k\sigma}\rangle$和原子为中心的局域部分。离域部分可以用平面波表示，其投影到实空间为
\begin{equation}
	\langle\vec r|\tilde{\Psi}_{i\vec k\sigma}\rangle=\dfrac1{\sqrt{\Omega_r}}\sum_{\vec G}C_{i\vec G\vec k}^{\sigma}(\vec r)\mathrm{e}^{\mathrm{i}(\vec G+\vec k)\cdot\vec r}
	\label{eq:PAW-eq-2}
\end{equation}
这里$\vec r$表示实空间坐标，$i$是能带标记，$\sigma$是自旋标志，$\Omega_r$是实空间体积，$\vec G$是平面波的波矢，$\vec k$是\textrm{KPOINTS}中规定的简约波矢，$C_{i\vec G\vec k}^{\sigma}(\vec r)$是平面波的展开系数。

当方程\eqref{eq:PAW-eq-2}中的平面波是完备的，方程\eqref{eq:PAW-eq-1}原则上是精确的。由于在实空间中，临近原子核的局域部分存在显著振荡由原子波函数表示，因此实际操作中，可以截断平面波的高阶部分:
\begin{displaymath}
	\dfrac12|\vec G+\vec k|^2<E_{\mathrm{cutoff}}
\end{displaymath}
原则上，截断能量主要由赝势，当然也与特定体系和性质确定的。因此为了确定合适截断能，开始计算时必须有收敛测试。

典型的做法是按不同的截断能量计算与体系总能作图，可以确定截断能的收敛情况。截断能参数\texttt{ENCUT}确定了基函数的大小，截断能对总能的收敛一般比较慢，可以选择关心的物理量，如体系的振动频率、化学移动、带隙等随着体系截断能收敛的情况。在\textrm{VASP}中，计算物理量的精度主要由平面波的基组确定，但原则上如果不是收敛检验，没有办法指定需要的截断能，所以最保险的办法，是和总能对截断能收敛检测类似，进行物理量对参数\texttt{ENCUT}的收敛检测。

\ch{H2O}的振动频率:~采用有限差分和密度泛函微扰理论\textrm{(Density-functional pertubation theory, DFPT)}。

有限差分方法和\textrm{DFPT}都可以用于计算两阶导数、\textrm{Hessian}矩阵，声子频率。如果作用于电子的可微的势函数$V(\vec r;\mathbf{R})$中，离子位置$\mathbf{R}=\{\mathbf{R}_{\mu}\}$是参数，由\textrm{Hellmann-Feynman}定理，原子受力和\textrm{Hessian}矩阵表示为
\begin{displaymath}
	\begin{aligned}
		\dfrac{\partial E}{\partial R_{\mu i}}=&\int\dfrac{\partial V(\vec r;\mathbf{R})}{\partial R_{\mu i}}n(\vec r;\mathbf{R})\mathrm{d}\vec r \\
		\dfrac{\partial^2E}{\partial R_{\mu i}\partial R_{\nu j}}=&\int\dfrac{\partial^2V(\vec r;\mathbf{R})}{\partial R_{\mu i}\partial R_{\nu j}}n(\vec r;\mathbf{R})\mathrm{d}\vec r + \int\dfrac{\partial V(\vec r;\mathbf{R})}{\partial R_{\mu i}}\dfrac{n(\vec r;\mathbf{R})}{\partial R_{\nu j}}\mathrm{d}\vec r
	\end{aligned}
\end{displaymath}
这里$\mu$、$\nu$是离子位置，$i,j=x,y,z$，$n(\vec r;\mathbf{R})$是电荷密度。电荷密度的线性变化，可以表示为
\begin{displaymath}
	\Delta^{\mathbf{R}}n(\vec r;\mathbf{R})=2\Re\sum_{i,\sigma}^{\mathrm{occ}}\Psi^{\ast}_{i\sigma}(\vec r;\mathbf{R})\Delta^{\mathbf{R}}\Psi^{\ast}_{i\sigma}(\vec r;\mathbf{R})
\end{displaymath}
这里$\Delta^{\mathbf{R}}$是描述离子偏离的有限差分算符，其定义为:
\begin{displaymath}
	\Delta^{\mathbf{R}}f=\sum_{\mu}\sum_{i=x,y,z}\dfrac{\partial f}{\partial R_{\mu i}}\Delta R_{\mu i}
\end{displaymath}

\textcolor{red}{简要回顾\textrm{DFPT}}，根据一阶微扰理论，\textrm{Kohn-Sham}方程解波函数的变化为$\Delta^{\mathbf{R}}\Psi_{i\sigma}(\vec r;\mathbf{R})$满足\textrm{Sternheimer}方程
\begin{displaymath}
	
	(H-\epsilon_{i\sigma})|\nabla^{\mathbf{R}}\Psi_{i\sigma}\rangle=-(\Delta^{\mathbf{R}}V-\Delta^{\mathbf{R}}\epsilon_{i\sigma})|\Psi_{i\sigma}\rangle
\end{displaymath}
这里$H$是未经微扰的\textrm{Hamiltonian}，满足
\begin{displaymath}
	H=-\dfrac{\hbar^2}{2m}\nabla_{\vec r}^2 + V(\vec r;\mathbf{R})
\end{displaymath}
并有
\begin{displaymath}
	\Delta^{\mathbf{R}}V(\vec r;\mathbf{R})=\Delta^{\mathbf{R}}V_{\mathrm{ext}}(\vec r;\mathbf{R})+e^2\int\dfrac{\nabla^{\mathbf{R}}(\vec r^{\prime};\mathbf{R})}{|\vec r-\vec r^{\prime}|}\mathrm{d}\vec r^{\prime}+\dfrac{\mathrm{d}v_{xc}(n)}{\mathrm{d}n}\bigg|_{n=n(\vec r;\mathbf{R})}\Delta^{\mathbf{R}}n(\vec r;\mathbf{R})
\end{displaymath}
且$\Delta^{\mathbf{R}}\epsilon_{i\sigma}=\langle\Psi_{i\sigma}|\Delta^{\mathbf{R}}V|\Psi_{i\sigma}\rangle$。注意到根据有限差分定义的$\Delta^{\mathbf{R}}V$和这这里定义的$\delta^{\mathbf{R}}\Psi_{i\sigma}\rangle$，可以求解\textrm{Sternheimer}方程得到$\Delta^{\mathbf{R}}\Psi_{i\sigma}$，由此可以更新$\Delta^{\mathbf{R}}n$，当达到自洽迭代，力和\textrm{Hessian}矩阵就可以算出来了。

\textcolor{red}{\textrm{DFPT}比有限差分方法的优势在与只需要计算占据态。此外，\textrm{DFPT}还有一个优点，就是不同波长的扰动部分是解耦的，因此\textrm{DFPT}可以不用采用超晶胞，即可计算任意波矢$\vec q$的声子频率。不过\textrm{VASP}中没有采用\textrm{DFPT}，因此孤立分子的振动频率计算与该方法无关。}

在参数设置文件\textrm{INCAR}中，\texttt{IBRION}=6表示采用有限差分方法，而\texttt{IBRION}=8表示采用\textrm{DFPT}理论。注意在\textrm{INCAR}文件中，完成能量截断的收敛测试后，设置参数\texttt{ENCUT}，以及\texttt{PREC}=\textrm{Accurate}。注意，有限差分方法中，需要有未占据能级的贡献，而\textrm{DFPT}中只需要占据能级的贡献，\textcolor{red}{因此\texttt{IBRION}=6时，需要设置计算参数\texttt{NBANDS}。}

对于非共线的$N$-原子分子或晶体体系，共有$3N$个振动模式。具体到\ch{H2O}，只有3个非0频率模式，包括平动、振动和转动模式、其余频率模式则为0。

采用线性响应\textrm{(DFPT)}方法(即\texttt{IBRION}=8，\texttt{EDIFF}=\textrm{1E-8})的结果为参考，通过参数\texttt{POTIM}改变步长，检验有限差分方法计算频率对时间步长的影响。

在输出文件\textrm{OUTCAR}中，在每个振动频率下，依次给出的\texttt{X,Y,Z}是离子位置，\texttt{dx,dy,dz}表示分子中离子受力值的漂移，每一行是一个原子。

\textcolor{red}{计算过程中注意检验\textrm{INCAR}文件中截断能\texttt{ENCUT}=529和\texttt{ENCUT}=800的收敛情况和对应的频率变化。具体到\ch{H2O}中，当的漂移过大时，0频率振动模就很难算准。此时最简单的办法是将能量截断参数\texttt{ENCUT}提升到800\textrm{eV}；不过，真正有意义的振动频率，反而对能量截断参数不敏感。}

\ch{H2O}的对相关函数\textrm{pair correlation function}
经典的分子动力学模拟，主要就是特定温度下，根据\textrm{Newton}第二定律，模拟组成分子的原子运动状况。换言之，在分子动力学模拟中，每个离子步迭代，模拟的是一个时间步长，在这个过程中，原子作为经典粒子，受力运动。如果原子的受力是通过量子力学的第一原理方法得到的，就称为第一原理分子动力学\textrm{(ab-initio MD)}。

第一原理分子动力学模拟中，如果缓慢降低温度，也可以用来求解离子弛豫问题。与阻尼分子动力学\textrm{(damped MD)}中引入阻尼函数类似，这样的方法称为模拟退火方法\textrm{(simulated annealling approach)}。该方法特别适合于自由度$\geeslant20$的，有多个振动频率的体系。

该方案模拟的\ch{H2O}分子的弛豫，比起采用\texttt{RMM-DIIS}方法更准确。第一原理分子动力学优于\textrm{RMM-DIIS}和\textrm{CG}算法在于，其模拟的原子径迹\textrm{(trajectories)}，即每个时间步上离子的位置连线，是有物理意义的。该方法还可以计算对相关函数，该函数描述的是，在一个给定的距离内，以一个粒子的视角位置，找到另一个粒子的概率。

这里学习的是，\textrm{VASP}计算中通过\textrm{Nos\'e-Hoover}等温方法实现温度控制并完成分子动力学计算。
输入控制文件\textrm{INCAR}

计算结果采用\textrm{Py4vasp}，将每个时间步长下的能量可视化。

由于模拟过程中温度的影响，体系的总能会表现出震荡。因为\textrm{Nos\'e-Hoover}恒温模拟过程中，每个时间步长和温度控制中，离子位置会有微小的移动。完成离子弛豫后，获得新的体系总能。因为计算过程中起始和终了温度是相同的，整个计算中，位置的振动和总能的振动遵从相同的分布规律。

采用\textrm{Py4vasp}完成分子动力学模拟的离子位置移动的可视化。

%对相关函数通过在一个给定的距离内，以一个粒子的视角位置，找到另一个粒子的概率。
\textrm{VASP}计算的对相关函数的数，写入文件\textrm{PCDAT}。当粒子间距离较大时，对相关函数即体系的密度。这里的晶胞参数为
$0.52918\times12=6.35916\AA$，因此根据对相关函数的形状，就可以估计两个粒子间开始发生相互作用的起始位置。对相关函数当距离小于6\AA时，出现相互作用，即\ch{H2O}中\ch{H}和\ch{O}的距离小于6\AA时，发生相互作用。

在短程距离处，对相关函数描述的是一个\ch{H2O}分子中，原子间相互作用的形式。

\textcolor{red}{\ch{H2O}分子的对相关函数中第一个峰位置约1\AA，第二个峰大约位于1.53\AA。考虑到其振动模式中，最主要的是对称弯曲振动，采用振动模式计算估计，峰位在1.53\AA附近裂分。}

\textcolor{red}{当\ch{H2O}分子处于基态，\textrm{H-O}的键长接近0.97\AA，而H原子间间距为1.53\AA。分子振动的存在，会引起振动峰的扩展。因此这里振动模式引起的是峰的劈裂。根据这里的计算，有一个频率为$3853.203528\mathrm{cm}^{-1}$，折算到\AA单位，有$1/3.853203528=0.2595243134013865\AA$，振动频率引起的两个峰的距离一致。}

\subsection{体相材料计算}\label{Sec:bulk}
对于周期体系，考虑空间对称性，共有7种晶系\footnote{7种晶系:~三斜晶系、单斜晶系、正交晶系、四方晶系、六方晶系和立方晶系}，7种晶系结合带心结构(带心结构包括:~底心、体心和面心三类)可以存在14种\textrm{Bravais}格子。周期体系的最小重复单元称为初基原胞\textrm{(primitive cell)}。早\textrm{VASP}计算中，常用的晶胞参数就是用初基原胞三个基本矢量$\vec a,\vec b,\vec c$。

与周期体系的晶胞密切关联的，是实空间密切关联的\textrm{Fourier}变换的倒空间(也称$\vec k$空间)晶胞。$\vec k$空间的初基原胞称为第一\textrm{Brillouin}区。实际的计算中，还有一个重要的概念，就是不可约\textrm{Brillouin}区，不可约\textrm{Brillouin}区是体系所属的点群对称操作将第一\textrm{Brillouin}区分割后的最小等价区域。习惯上，用户选择不可约\textrm{Brillouin}区中的高对称性的线和点，根据这些线和点能量本征值，绘制体系能带图。一般这些高对称性线和点，都有专门的名称，如$\Gamma$点，就是第一\textrm{Brillouin}区的原点。

结构文件\textrm{POSCAR}

在\textrm{POSCAR}文件中，惯用的面心立方\textrm{(Face-Centered Cubic, FCC)}对称性中可以划分出更小的重复单元。因此初基原胞的晶胞参数可以确定为
\begin{displaymath}
	\begin{aligned}
		\vec a =\dfrac{a}2(\hat{\vec x}+\hat{\vec y})\\
		\vec b =\dfrac{a}2(\hat{\vec y}+\hat{\vec z})\\
		\vec c =\dfrac{a}2(\hat{\vec z}+\hat{\vec x})\\
	\end{aligned}
\end{displaymath}

在惯用原胞的立方结构中，共有8个顶点和6个面。考虑到顶点和面心原子的共享特征，因此每个惯用原胞中的贡献分别是1/8和1/2，由此惯用原胞中的重复单元的数目为：
\begin{displaymath}
	N=8\times\dfrac18+6\times\dfrac12=4
\end{displaymath}
而初基原胞中的最小重复单元为1。

\textcolor{red}{在\textrm{POSCAR}文件中，第一行是体系名称，第二行是通用的标度参数\textrm{(scaling parameter)}$a$。第三到第五行分别是矢量$\vec a/a,\vec b/a,\vec c/a$(因此每个矢量值都要乘通用晶格系数)。}

计算使用的输入控制参数文件\textrm{INCAR}

在控制参数文件\textrm{INCAR}文件中，\textrm{VASP}默认会首先查找是否存在\textrm{WAVECAR}文件，并由此为起点开始计算。所以如果用户要向重新开始新的计算，建议删除\textrm{WAVEVAR}。\textcolor{red}{\textrm{VASP}计算的初始电荷密度，来自体系各原子电荷密度的叠加}，在重启的\textrm{VASP}计算中，还可以通过参数\texttt{ISTART}和\texttt{ICHARG}控制计算使用的初始电荷密度来源。

选择的$\vec k$空间布点方案，保存在\textrm{KPOINTS}文件中
$\vec k$点的生成是采用\textrm{Monkhorst-Pack}模式，均匀剖分第一\textrm{Brillouin}区实现的。如果在\textrm{Brillouin}区各方向上的布点都是奇数，就可以保证产生的是$\Gamma$-点位中心的网格布点。

为了测试体系总能随不同晶胞参数的变化，应用\textrm{bash}脚本修改\textrm{POSCAR}文件中通用控制参数。这里通用参数由3.5到4.3，步长为0.1。\textcolor{red}{对应的脚本为}

各种不同晶胞参数的\textrm{VASP}计算，得到的总自由能数据存在\textrm{OSZICAR}文件中。完成所有\textrm{VASP}计算后，将不同晶格常数和响应的能量存在\textrm{loop\_lattice\_constant.dat}文件中。

根据文件\textrm{loop\_lattice\_constant.dat}中的数据，\textcolor{red}{绘制晶格-总自由能曲线。}

扫描晶胞参数，总能最小的位对应晶胞参数为$3.9\AA$。因此\textcolor{red}{基态结构的\textrm{POSCAR}文件}为：

\textbf{面心立方\rm{Si}的的态密度计算}
\textcolor{red}{为了节约计算开销，\textrm{VASP}会根据体系所属的点群和空间群，将体系划分为更小的重复单元；相应地，\textrm{KPOINTS}文件规定的倒空间剖分的$\vec k$点网格也将约化为更少的不可约网格点。}

输入控制文件\textrm{INCAR}中

计算控制参数\texttt{ISMEAR}=-5，表示$\vec k$空间网格积分方案采用校正的\textrm{Bl\"ochl}方案。有必要指出的是，\textcolor{red}{对于占据数变化，该方法是不可变分的，因此对不部分占据的能带会产生错误的力和应力。}因此，特别要注意，对于导体体系，几何结构优化时，不要使用该参数设置。

计算控制参数\textrm{LORBIT}=11，则会写入详细的\textrm{DOSCAR}文件。

\textcolor{blue}{$\vec k$空间剖分文件\textrm{KPOINTS}文件中，\textrm{VASP}采用\textrm{Monkhorst-Pack}方法实现网格剖分，然后利用倒空间所属点群对称性，确定等价点和等价点权重，后续对$\vec k$空间\textrm{Brillouin}区的积分变成对不可约\textrm{Brillouin}区的$\vec k$点与权重的求和。

习惯上，$\vec k$空间的网格剖分为$\Gamma$点为中心，各个方向的$\vec k$点并不要求完全一样多。}

\textrm{DOS}计算中，如果增加$\vec k$网格点，则能产生更平滑的\textrm{DOS}图形，在几何结构优化中，可以用少量$\vec k$点来计算体系总能。

\textcolor{blue}{运行\textrm{VASP}}

检查输出文件\textrm{OUTCAR}中的$\vec k$点的相关信息:~对于$15\times15\times15$的$\vec k$空间网格剖分，\textrm{OUTCAR}中列出120个不可约$\vec k$点及其权重，权重是$\vec k$点在第一\textrm{Brillouin}区内的等价$\vec k$点数目。对第一\textrm{Brillouin}区的3375个$\vec k$点的求和，等价于120个不等价$\vec k$点的权重求和。

\textcolor{blue}{运行\textrm{py4vasp}将\textrm{DOS}可视化}

图中给出的是\textrm{Fermi}能级附近的高分辨的\textrm{DOS}图，根据习惯，在\textrm{DOS}中将\textrm{Fermi}能级设置为0。为了得到更高分辨的\textrm{DOS}图，在指定能量范围$-15\mathrm{eV}\sim2\mathrm{eV}$内，需要采用更多$\vec k$点。\\为了确定能量范围，在\textrm{OUTCAR}中找到收敛结果中的\textrm{Fermi}能级，

\textcolor{blue}{\textrm{Fermi}能级为}

在\textrm{INCAR}中设置控制参数\texttt{NEDOS}，\texttt{EMIN}和\texttt{EMAX}，重新计算。

应用\textrm{py4vasp}重新绘制体系的\textrm{DOS}，注意\texttt{EMIN}和\texttt{EMAX}是能量上下限，并非是相对\textrm{Fermi}能级设置的能量参数。

%沿特定高对成性方向$L-\Gammma-X-U$和$K-\Gamma$绘制\textrm{FCC}\ch{Si}的能带图
用于能带绘制的高对称性$\vec k$点路径的选择，依赖于所选择的空间群。有很多工具可以用来确定\textrm{Brillouin}区的空间群并提取$\vec k$点路径。\textcolor{blue}{一个在线的工具是\textrm{SeeK-path}\footnote{\url{https://seekpath.materialscloud.io/}}，该工具接受\textrm{POSCAR}文件。}，确定$\vec k$点路径上的各$高对称性\vec k$点坐标，并写入\textrm{KPOINTS}文件中。

原则上，计算能带结构会需要5个输入文件，除了4个常用的\textrm{POSCA,INCAR,KPOINTS,POTCAR}，额外需要\textrm{CHGCAR}文件来完成自洽计算(\textrm{CHGCAR}采用\textrm{DOS}中的结果).

输入控制文件\textrm{INCAR}中

这里控制参数\texttt{ICHARG}=11，表明计算用的电荷密度直接由\textrm{CHGCAR}中读取，并且在计算过程中保持电荷密度固定。

能带$\vec k$点路径的\textrm{KPOINTS}文件：

执行\textrm{VASP}计算

并用\textrm{py4vasp}模块绘制能带图形。

\subsection{结构弛豫与收敛测试}
除了通过改变晶胞参数来确定搜索体系最稳定的基态，\textrm{VASP}还可以通过\texttt{ISIF}参数，来实现体系的结构弛豫计算。这一部分将以金刚石型\ch{Si}和高压下金属相体心四方结构的\ch{Si}为例，说明在\textrm{VASP}计算中，如何较为合理地实现结构弛豫与参数收敛测试。

\textcolor{red}{首先采用借助脚本改变晶胞参数而不使用控制参数\texttt{ISIF}计算初基原胞中包含多个原子的\ch{Si}(金刚石型)}

有必要指出的是，通过\textrm{DFT}计算获得的弛豫(优化)基态，有时候也可能是某种亚稳态\textrm{(metastable state)}而非真实基态。出现这种情况，往往是因为计算的某些初始条件，偏离基态较远，\textrm{DFT}计算收敛到了局域极小位置。除了基态总能，这种现象还包括晶体结构、磁构象等性质，同样存在类似现象。所以对于计算结果，最好能与实验结果相对照，彼此印证。

另一方面，与基态结构相比，\textrm{DOS}对截断能的敏感程度要低一些。因此，计算\textrm{DOS}时，可以适当调低截断能。

\textcolor{blue}{应用\textrm{CG}方法完成金刚石结构的\ch{Si}的弛豫}

前面已经讨论过，\textcolor{red}{\textrm{VASP}计算中赝波函数是用平面波展开的，由于能量截断，赝波函数展开是不完备的。一个典型的结果，就是在结构弛豫中，晶胞会比实际的偏小，这称为\textrm{Pulay}应力。}换言之，同样的\textrm{VASP}计算截断能，体系的基态体积越小，越有利于得到稳定的基态。

\textrm{Pulay}应力是非物理的，计算中应该尽可能的消除或降低，习惯上的办法是，结构优化时，适当选取高一些的截断能，并且一旦发现优化过程中，体积变化较大时，选择合适的截断能，重新开启结构优化。

理论上，输入控制文件中引入晶胞弛豫控制参数\texttt{ISIF}=3，在指定截断能\texttt{ENCUT}条件下直接优化计算对象的晶胞参数，可以将体系的结构弛豫到当前最稳定的基态。但正如此前讨论的，一般晶胞体积改变，意味着\textrm{Pulay}应力的贡献也会变化。更稳妥的办法是，利用脚本选择不同的晶格参数，选择晶胞弛豫参数\texttt{ISIF}=4，结构弛豫算法\texttt{IBRION}=2来弛豫晶胞的形状和离子位置坐标。因为这样的结构弛豫，晶胞的体积改变小，结构弛豫的基函数维度不变，优于\texttt{ISIF}=3条件下的截断能不变。而且如果结构弛豫过程中，一旦晶胞的形状或体积发生显著变化时，基函数(截断能)也应该作相应的调整。在这种情况下，就可以停止弛豫过程，将\textrm{CONTCAR}文件的数据取代\textrm{POSCAR}文件的数据，重启并继续结构弛豫。

为了实现\textrm{VASP}计算任务按需要随时起止，重启任务又不必再从头计算，\textrm{VASP}的处理方法是，用户在认为有必要停止的时候，在执行目录下生成空文件\textrm{STOPCAR}。这样做最大的好处是不同于直接执行\textrm{kill}命令(或作业管理系统的\textrm{cancel}命令)，强行终止正在执行的\textrm{VASP}进程，而是等到当前离子步执行完毕后，保留中间数据，用于后续计算。在这种情况下，同样用\textrm{CONTCAR}的内容取代\textrm{POSCAR}，并将\textrm{INCAR}中的控制参数\texttt{ISTART}=0和\texttt{ICHARG}=2并选择适当的\texttt{ENCUT}，在移除\textrm{STOPCAR}文件后，重启弛豫过程。

\textcolor{red}{以本算例为例，如果\textrm{INCAR}中参数\texttt{NSW}=10}，全部执行完毕，10个离子步后，结构弛豫结束，但体系尚未达到最稳定的基态。这时需要将\textrm{CONCAR}数据取代\textrm{POSCAR}中，然后重启弛豫计算进程。利用\textrm{STOPCAR}文件控制计算进程，原则上停止的标准是任意的，如果您在本例中第三个离子弛豫步结束前引入\textrm{STOPCAR}文件，则可以预见，如果在此基础上接续\textrm{VASP}计算，则将在第七步时得到与之前一致的结果。如果想要得到更好的结果，则可在\textrm{INCAR}设置控制参数\texttt{NSW}超过7即可。

在结构弛豫计算中，剖分$\vec k$空间的网格点密度也会对结构弛豫有所影响。

剖分$\vec k$空间网格的数目，主要由晶胞结构文件\textrm{POSCAR}确定，但有时也会与体系所计算的材料物性有关，如需要考虑长程相互作用，$\vec k$点网格需要密一些。因此，原则上\textrm{VASP}计算前，应该做一下$\vec k$点收敛测试。

\textcolor{red}{对高压下金属态\ch{Si}(\ch{Si}\textrm{-II})的结构优化，考察$\vec k$空间网格布点的影响}

\textcolor{red}{通过脚本控制$\vec k$空间网格密度}，结构弛豫控制参数\texttt{ISIF}=7，只优化晶胞的体积，不改变晶胞的形状和其中原子的位置。体积收敛的标准是
\begin{itemize}
	\item 体系总能误差收敛到2\textrm{meV}以内;
	\item 晶格的第一个矢量变化的精度范围达到0.005\AA.
\end{itemize}
换言之，收敛的状态可以通过总能和晶格参数对$\vec k$空间网格点作图确定。

\textcolor{red}{一个可行的解决方案}
输入控制文件\textrm{INCAR}

控制$\vec k$空间网格布点密度的\textrm{bash}脚本

用于绘制检验$\vec k$空间布点收敛图线的\textrm{gnuplot}脚本

注意随着$\vec k$点增加，总能和晶格参数并不单调地收敛到某一数值，而是表现出数值方差\textrm{(variance)}随着$\vec k$点的增加不断下降。而且有必要指出的是，如果$\vec k$空间网格点中是否包括$\Gamma$点，结果存在明显差别。

为了计算体系\textrm{DOS}，以确定的最稳定基态晶胞为起点，选择$\vec k$空间布点数为$18\times18\times18$(因为当前体系为金属态)，并在\textrm{INCAR}中添加设置控制参数\texttt{LORBIT}=11。

为了对比$\vec k$空间积分方案中\textrm{Gaussian}积分与四面体积分的差别，分别采用两种方法完成\textrm{DOS}计算并绘图。

相关控制参数文件和绘图脚本分别为：

\subsection{面心立方结构的极化计算:~\ch{Ni}}
自旋极化\textrm{DFT}(Spin-Polarized DFT, SDFT)中，将体系中自旋向上\textrm{(spin-up)的电子和自旋向下\textrm{(spin-down)}的电子解耦分别处理，因此不同自旋的电子可以有不同的本征值。不同自旋的电子间相互作用，体现在交换-相关泛函里。具体计算中，在\textrm{INCAR}文件中，使用控制参数\texttt{ISPIN}=2，即可打开自旋极化计算，同时会自动计算体系的共线磁矩\textrm{(collinear magnetic moments)}。如果需要考虑旋-轨耦合\textrm{(spin-orbit coupling, SOC)}，则需要设置控制参数\texttt{LSORBIT}=\textrm{True}，并在执行计算时使用非共线磁矩版命令\textrm{vasp\_ncl}。

结构弛豫、\textrm{DOS}和能带计算的输入文件\textrm{INCAR}和$\vec k$空间网格剖分文件\textrm{KPOINTS}。

因为已确定晶胞为面心立方\textrm{(Face-centered Cubic, FCC)}，因此\textrm{INCAR}文件中弛豫控制参数为\texttt{ISIF}=7，而弛豫过程中离子步上限为\texttt{NSW}=6，因为弛豫过程中晶胞体积变化较大，连续执行的离子步过多，可能会由于\textrm{Pulay}应力导致优化后的体积偏小。为了避免出现这种情况，当前的做法较为稳妥的，将当前结束得到的\textrm{CONTCAR}复制到\textrm{POSCAR}中，重新开启弛豫，直到晶胞到达最稳定基态的结构。用户在计算过程中可以通过\textrm{vimdiff}命令，对比两者在小数点后第几位开始变换。

优化后的晶格参数
\begin{displaymath}
	3.5\times0.5025/0.5=3.517\AA
\end{displaymath}

计算面心立方\ch{Ni}的态密度，用\textrm{py4vasp}绘图，不同自旋分别展现可有:

相应地，能带计算得到的能带为：

\subsection{能带和磁矩\rm{DFT+}$U$:~反铁磁\ch{NiO}}
反铁磁\ch{NiO}属于强关联体系，因为\ch{Ni}原子含有局域的3\textit{d}电子。含有过渡金属的氧化物的能带结构很难用标准的\textrm{GGA}交换-相关泛函，如\textrm{PBE}来准确描述，采用这些泛函得到的电子结构和磁学性质都是不正确的。

因此，在\textrm{DFT}框架下，必须采用更先进的泛函来处理强关联体系，包括\textrm{DFT}+$U$，\textrm{meta-GGA}泛函(如:~\textrm{SCAN}和\textrm{MBJ})，杂化泛函(如:~\textrm{HSE06})。\textcolor{red}{更精确的方案是超越\textrm{DFT}的方法，如动力学平均场理论\textrm{(Dynamical mean-field theory, DMFT)}}。

以下分别采用各类泛函模拟\ch{NiO}，并对比结果的带隙和\ch{Ni}原子的磁矩，并与实验结果的值(分别为$4.0\sim4.3\mathrm{eV}$和$1.9\sim2.2\mu_{\mathrm{B}}$)对比。

结构文件

不同交换-相关泛函的输入文件
\begin{itemize}
	\item 标准\textrm{DFT}:~\textrm{PBE}
	\item \textrm{DFT}+$U$计算
	\item \textrm{meta-GGA}:~\textrm{SCAN}
	\item \textrm{meta-GGA}:~\textrm{MBJ}
	\item \textrm{hybrid}:~\textrm{HSE06}
\end{itemize}

$\vec k$空间剖分网格
\begin{itemize}
	\item \textrm{KPOINTS}
	\item \textrm{HSE06/KPOINTS}
\end{itemize}

\ch{NiO}是\ch{NaCl}结构，在沿[111]轴的铁磁有序态的面堆积基础上，构成反铁磁矩，构造形成\textrm{AFM II}反铁磁序态，由此可以划分出最小晶胞是菱形三方晶胞。以下分别给出面心立方和菱形三方晶胞的图示，蓝色是\ch{Ni}原子，箭头表示磁矩方向，\ch{O}原子是红色的。

在面心立方晶胞中，包含的原子数目为
\begin{displaymath}
	1+8\times\dfrac18+6\times\dfrac12+12\times\dfrac14=8
\end{displaymath}
类似的，可以算出菱形三方晶胞的原子数目为4。需要指出的是，在\ch{NaCl}结构中的初基原胞中含有\ch{Ni}和\ch{O}两类原子。

对于考虑自旋极化的磁性材料，需要特别注意，可以通过控制参数\texttt{MAGMOM}来设置磁性原子的初始构型。一般对磁性材料来说，初始构象往往会对计算模拟最终得到的磁性结果(如:~铁磁与反铁磁等)有影响。所以如果计算时对初始构象有特定要求，就应该通过控制参数\texttt{MAGMOM}来据此设置。控制参数\textrm{LORBIT}会将临近原子附近的局域磁矩投影分配到各轨道，并将结果写入\textrm{OUTCAR}文件中。

在\textrm{VASP.6.4.3}，允许用户使用控制参数\textrm{XC}来指定计算所使用的交换-相关泛函。由于部分地引入\textrm{Hartree-Fock}交换项的贡献，含有杂化泛函的计算比起\textrm{GGA}和\textrm{meta-GGA}泛函的计算要复杂得多，\textcolor{red}{所以减少计算开销，对使用\textrm{HSE06}泛函的计算，使用更少一些的$\vec k$点，而随着$\vec k$空间网格剖分更粗略，带隙的误差也会存在0.2\textrm{eV}的涨落。}

\textrm{PBE}是周期体系\textrm{DFT}计算最常用的标准交换-相关泛函，主要用于和体系总能有关的计算，如几何结构优化、形成能或成键能等。

\textcolor{red}{应用\textrm{py4vasp}模块，可以提取能带的带隙信息}:\\
\textcolor{red}{\textrm{PBE}计算的带隙为约0.96\textrm{eV}，远低于实验观测的$4.0\sim4.3\mathrm{eV}$的带隙值。}

\textrm{INCAR}文件中，控制参数\texttt{BANDGAP}，将在\textrm{OUTCAR}文件中输出带隙。

半导体带隙的分类
\begin{itemize}
	\item 直接带隙:~价带顶与导带底对应同一个$\vec k$点位置；
	\item 非直接带隙:~价带顶与导带底不在同一了$\vec k$点位置。
\end{itemize}

\textcolor{red}{除了最小带隙，\textrm{py4vasp}模块还可以提取能带的最小直接带隙，以辅助用户判断带隙类型}:\\
\ch{NiO}的最小直接带隙是1.126\textrm{eV}，比最小带隙大约0.17\textrm{eV}，因此反铁磁的\ch{NiO}的带隙为间接带隙。

如果\textrm{INCAR}参数中设置了控制参数\texttt{BANDGAP}，在\textrm{OUTCAR}文件中会给出体系的最小带隙，而且会给出价带顶和导带底各自对应的$\vec k$点位置：\\
\textcolor{red}{相应的命令是
	\textrm{grep -B3 -A2 "fundamental gap" OUTCAR}}

不难看出，反铁磁的\ch{NiO}能带中，两个不同自旋的能带是简并的。这也是反铁磁计算的基本设定。

应用\textrm{py4vasp}模块，还可以提取原子的磁矩信息：

不难看出，由于反铁磁要求，\texttt{MAGMOM}设定，两个\ch{Ni}原子的初始构型分别为自旋向上(+2)和自旋向下(-2)，因此计算最后得到的\ch{Ni}原子的磁矩也是自旋相反的，严格保持反铁磁熟悉。类似地，\ch{O}原子的磁矩均为0。通过\textrm{PBE}泛函得到\textrm{Ni}的磁矩大小\textrm{($1.37\mu_{\mathrm{B}}$)}比实验值\textrm{($1.9\sim2.2\mu_{\mathrm{B}}$)小得多。有必要指出的是，将计算得到的局域磁矩投影到原子轨道，其数值的精度非常依赖构建原子数据集的半径的大小。所以即使\textrm{DFT}计算得到的磁矩是精确的，投影到原子的上的磁矩大小，也很可能与实验值不一致。

在\textrm{GGA}计算基础上，检验各类修正后的交换-相关泛函的预测带隙和原子磁矩的能力，至少在定性结果上，有没有比简单的\textrm{GGA}有所改善，与实验结果相符合。

\begin{itemize}
	\item \textrm{DFT+}$U$\\
一般地，\textrm{DFT}+$U$中，在\textrm{LDA}或\textrm{GGA}泛函考虑交换-相关作用基础上，引入在位\textrm{(on-site)}的\textrm{Coulomb}和交换作用参数$U$，唯相地描述局域的\textit{d}或\textit{f}电子间相互作用。相比于各类改进的交换-相关泛函方法，\textrm{DFT+}$U$的计算开销最小，仅依赖唯相参数$U$即可修正部分电子的局域相关作用。但成也萧何，败也萧何，由于$U$是可调参数，带有很强的经验属性，因此\textrm{DFT+}$U$方法的普适性大大降低，也是其饱受诟病的根源。

本算例中，\ch{Ni}原子的参数$U=8.00~\mathrm{eV}$和$J=0.95~\mathrm{eV}$，计算得到的最小带隙为3.438~\textrm{eV}；\textcolor{red}{磁矩为}

\item \textrm{SCAN}泛函\\
	\textrm{SCAN}是周期体系计算中最常用的\textrm{meta-GGA}泛函。所谓\textrm{meta-GGA}，就是在\textrm{GGA}泛函包含密度梯度的贡献外，还包含了密度的二阶梯度的贡献，进一步提升交换-相关泛函的计算精度。因为密度的二阶梯度贡献主要来自动能密度，所以\textrm{meta-GGA}主要来自的是动能密度的贡献。\textrm{SCAN}泛函的优势在于完全不依赖经验参数，具有交换-相关泛函应有的全部解析特性。

	本算例中，\textrm{meta-GGA}泛函计算得到的最小带隙为2.581~\textrm{eV}；\textcolor{red}{磁矩为}

\item \textrm{MBJ}泛函\\
	\textrm{MBJ}是另一类\textrm{meta-GGA}泛函，在固体计算中，\textrm{MBJ}在固体能带的带隙计算表现出特殊的优势。但\textrm{MBJ}在准确估算较大带隙的同时，会在体系总能中引入额外的非物理势的贡献。因此，\textrm{MBJ}泛函无法与体系总能关系紧密的物理量计算中，如结构优化或求体系稳定的基态。

	本算例中，\textrm{meta-GGA}泛函计算得到的最小带隙为4.614~\textrm{eV}；\textcolor{red}{磁矩为}

\item \textrm{HSE06}泛函\\
	\textrm{HSE06}是屏蔽杂化泛函\textrm{(screened hybrid functional)}。杂化泛函中的``杂化''意味着在局域形式的泛函中，掺入了非局域的\textrm{Hartree-Fock (HF)}交换能的贡献。在周期体系中，考虑到多电子体系的屏蔽效应，交换-相关泛函的离域部分主要也来自电子的近程相互作用。因此，\textrm{HSE06}屏蔽了电子的长程相互作用部分的贡献，使得该泛函在$\vec k$空间网格上，比\textrm{PBE0}泛函有更快速的收敛属性。但是，也因为包含了\textrm{HF}交换能的贡献，考虑杂化泛函后的计算开销比一般\textrm{GGA}泛函计算要高1-2个数量级。

	本算例中，\textrm{meta-GGA}泛函计算得到的最小带隙为4.788~\textrm{eV}；\textcolor{red}{磁矩为}
\end{itemize}
总的来看，\textrm{DFT+}$U$，\textrm{MBJ}和\textrm{HSE06}对能带的带隙计算都较为精确。但是\textrm{DFT+}$U$对带隙表现出一定程度的低估，而\textrm{MBJ}和\textrm{HSE06}对带隙表现出一定程度的高估。

在\ch{Ni}的原子磁矩计算中，\textrm{DFT+}$U$和\textrm{MBJ}得到的结果接近$1.73\mu_{\mathrm{B}}$，与实验值吻合得最好，但仍比实验值偏小。有必要指出的是，当前计算的磁矩是\ch{Ni}的\textit{d}电子自旋部分的贡献，尚未包含轨道磁矩的贡献。在\ch{NiO}中，轨道磁矩的贡献大约为$0.3\sim0.5\mu_{\mathrm{B}}$。

\subsection{\rm{Van der Waals }力}
\textcolor{red}{考虑\textrm{Ven der Waals}相互作用后，采用\textrm{Tkatchenko-Scheffler}方法计算石墨\textrm{graphite}的层间相互作用。}

\textrm{GGA}泛函的半局域\textrm{(semilocal)}特征，使得它常常低估长程相互作用。以石墨的模拟为例，\textrm{PBE}泛函估计得层间相互作用能为$\sim1\mathrm{meV/atom}$，相比于无相规近似\textrm{(Random Phase Approximationg, RPA)}的值$0.048~\mathrm{eV/atom}$\upcite{PRL105-196401_2010}小得多。所以在需要在\textrm{GGA}泛函基础上，寻找可以考虑\textrm{Van der Waals}相互作用贡献的形式。\textrm{Tkatchenko-Scheffler}方法基于1997年\textrm{Becke}的幂级数假设，为实现原子对\textrm{(atom-pairwise)}之间的色散作用校正\textrm{(dispersion correction)}，引入阻尼函数\textrm{damped function}来处理短程作用的一套方法。该方法额外引入的解析的函数形为$C_6\cdot R^{-6}$，用于原子对的色散校正项的计算。\upcite{PRL102-073005_2009,PRB87-064110_2013}。

石墨烯\textrm{(Graphene)}的结构文件

石墨的结构文件

计算控制文件\textrm{INCAR}

$\vec k$空间网格剖分文件\textrm{KPOINS}

在石墨烯中，$z$方向没有相互作用，因此$z$向的$\vec k$空间布点为1.

在\textrm{INCAR}中，因为本次计算主要是体系总能，不需要计算\textrm{DOS}和能带结构，因此计算控制参数\texttt{LWAVE}和\texttt{LCHARG}都设置为\textrm{FALSE}，这样计算过程中不会有数据输出到\textrm{WAVE}、\textrm{CHGCAR}中，适当节省计算资源和存储空间。

分别计算石墨烯和石墨体系。

完成后，可以通过\textrm{py4vasp}模块，实现$3\times3\times3$网格的石墨结构可视化。

不难看出，不同层的石墨的层间距有很大差别，为了得到每一个原子的层间成键能，一个简单而直白的办法，就是分别从石墨和石墨烯的\textrm{OUTCAR}文件中提取体系总能，然后将总能分别除以各种晶胞中的原子数目，两者的能量差可以认为是每个原子的层间成键能。

应用\textrm{bash}脚本提取体系总能，并计算
\begin{displaymath}
	\begin{aligned}
		&-37.42908542/4-(-18.54564785/2)\\
		=&-0.0844473~\mathrm{eV/atom}
	\end{aligned}
\end{displaymath}
\textcolor{red}{\textrm{Tkatchenko-Scheffler method}可以给出较为合理的稳定构型}，但是高估了原子的层间成键能。相比于\textrm{RPA}的参考值$0.048\mathrm{eV/atom}$，这种原子层间成键能的高估，部分原因是源于未考虑多体相互作用。为了校正这部分贡献，可以使用多体色散方法\textrm{(Many-body-dispersion-energy method, MBDM)}\upcite{PRL108-236402_2012}。

改变石墨和石墨烯计算的控制参数文件\textrm{INCAR}中\textrm{Van der Waals}的计算控制参数\texttt{IVDW}，包含多体校正，

再计算每个原子的层间成键能。

应用\textrm{Bash}脚本完成计算并提取体系总能，最后可得
\begin{displaymath}
	\begin{aligned}
		&-37.44318263/4-(-18.61910927/2)\\
		=&-0.0512410225\mathrm{eV/atom}
	\end{aligned}
\end{displaymath}
这样计算的每个原子的层间成键能与\textrm{RPA}参考值比较接近。

\textcolor{red}{采用\textrm{Tkatchenko-Scheffler}方法考虑\textrm{Van der Waals}相互作用，确定石墨层堆积方向的层间距}

原子间的长程的\textrm{Coulomb}相互作用不仅可以采用\textrm{Tkatchenko-Scheffler}方法计算，也可以采用\textrm{Ewald}求和计算。一般地，采用\textrm{Ewald}求和，为了避免奇点\textrm{(singularities)}的出现，将短程相互作用放在正空间处理，而将长程相互作用放在倒空间计算。

在本算例中，采用\textrm{Tkatchenko-Scheffler}方法计算，因为\textrm{GGA}泛函典型地会低估长程色散相互作用(如\textrm{Van der Waals}相互作用)，导致高估石墨层堆积方向层的晶格常数。比如，采用\textrm{PBE}泛函，计算得到的石墨的堆积层方向层间距离为8.84\AA，而实验值仅为6.71\AA。

结构文件\textrm{POSCAR}

计算参数控制文件\textrm{INCAR}

$\vec k$空间剖分网格文件\textrm{KPOINTS}

在\textrm{POSCAR}中，沿$z$方向是石墨层装堆积方向，该值是待优化的。

应用\textrm{Bash}脚本，选择$z$向层间距离范围$[6.6,6.8]$，变化步长为0.01。将其余自由度都固定为实验值，仅改变$z$方向的层间距离，得到一系列的$z$向值和总能的关系，绘制图形，可以确定优化的层间距离。

\textcolor{red}{\textrm{gnuplot}绘图脚本}

应用\textrm{Tkatchenko-Scheffler}方法得到的层间距离为6.65\AA，与实验值6.71\AA吻合得较好。而该方法估算的原子间层间成键能却高于\textrm{RPA}参考值。


%通过学习金属\textrm{Pt}原子基态能量的计算，可以掌握典型的\textrm{VASP}的主体流程\footnote{在所有计算之前，请确认\textrm{VASP}软件已经正确安装。}，了解体系基态能量最小化的基本算法，并熟悉基本的输入/输出文件的内容。此外，还可以了解如何在已完成计算的基础上，进行计算精度提升或完成后续计算等一系列处理方式。
%\subsection{输入文件}
