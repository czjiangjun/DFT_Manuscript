...
\url{https://vasp.at/tutorials/latest/}，这个教程在一直在维护和更新。笔者以为这是迄今为止最完整的\textrm{VASP}教程，除了对各算例有比较详细的输入、输出内容的说明，包括编译、安装及执行在内，都有详细的指导，有兴趣的\textrm{VASP}用户，特别是初学者，建议遵照教程尝试练习。本章也将参照教程的算例，对\textrm{VASP}具体计算中的内容作必要说明。另外，编著的\textrm{}也比较细致地描述了
\section{基本计算}
...
真空中的孤立原子的基态能量是\textrm{VASP}中最简单的算例。通过这个算例，可以掌握典型的\textrm{VASP}的主体流程\footnote{在所有计算之前，请确认\textrm{VASP}软件已经正确安装。}，了解体系基态能量最小化的基本算法，并熟悉基本的输入/输出文件的内容。
