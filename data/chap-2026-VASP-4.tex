...
标准输出中，在\textrm{entering main loop}前面，是计算资源配置和初始化；在这一行之后输出的内容与\textrm{OSZICAR}文件内容一致，每一列的含义请参阅\ref{subsectr:OSZICAR}的相关部分内容。
...
更完整的输出信息在\textrm{OUTCAR}文件中，当计算对象达到收敛后，\textrm{OUTCAR}中体系能量本征值，由于采用默认的低精度计算，读者的计算结果和这里的结果可能有微笑的数值差别:
{\fontsize{8.2pt}{6.2pt}\selectfont{
...
\subsection{原子/分子自旋极化计算}\label{Sec:atom-1}
