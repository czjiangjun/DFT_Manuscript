...
需要注意的是，用户不要随便改动\textrm{POTCAR}的内容。\textrm{VASP}官网明确指出，普通用户没必要了解\textrm{POTCAR}的详细内容，只需要注意\textrm{POTCAR}文件第4行\ch{O}元素的价电子构象(\textrm{O: s2p4})和第12行的参数\texttt{ZVAL}=6(表示\ch{O}元素的6个价电子)，两者是保持一致的。
...
由于\textrm{DFT}计算得到的简并$2p$轨道，会引入非物理的熵(\textrm{S})的贡献。参数\texttt{SIGMA}是计算引入的\textrm{Gaussian}展宽，它可以视为认为引入的电子温度\textrm{T}，小的\texttt{SIGMA}会降低熵的贡献，但可能会引起\textrm{Fermi}积分的变慢。这部分非物理的贡献就是自由能计算的熵的来源。
...
\begin{lstlisting}[language=Python, basicstyle=\ttfamily\small, keywordstyle=\color{blue}, stringstyle=\color{red!70!black}, frame=single]
import py4vasp
