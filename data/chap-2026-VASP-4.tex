...
按照迭代子空间直接反演\textrm{(Direct Inversion in the Iterative Subspace,DIIS)}最小化方法的原则，只要经过若干次迭代，就能获得体系的最小化，是一种直接求解的方法。在子空间得到最终结果。具体到\textrm{RMM}，这里最小化的是作用于原子的力而非体系的总能。对于一个作用于电子的连续的势函数$V(\vec r,\mathbf{R})$，它依赖于参数化的离子位置$\mathbf{R}=\{\mathbf{R}_{\mu}\}$，根据\textrm{Hellmann-Feynman}定理，原子的受力和\textrm{Hessian}矩阵的矩阵元可以表示为
\begin{displaymath}
	\begin{aligned}
...
\begin{displaymath}
	F_{\mu i}=\sum_{\nu j}H_{\mu i\nu j\partial R_{\nu j}}
\end{displaymath}
注意到\textrm{RMM-DIIS}方法会根据\textrm{Hessian}矩阵的特征值谱，把最小化问题，分解到不同的子空间来处理。
