...
\texrtrm{makefile.include}中有关预编译的变量主要为\\
\textrm{CPP}:~指定使用的预编译器
\begin{itemize}
...
$\vec k$空间布点方案是$4\times4\times4$网格点，如图\ref{NiO-LDA_U-KPOINTS}
\begin{figure}[h!]
\centering
\includegraphics[height=1.2in,viewport=0 0 100 110,clip]{NiO-LDA_U-KPOINTS.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{用于\textrm{LDA}+$U$的反铁磁\ch{NiO}计算的$\vec k$空间布点\textrm{KPOINTS}文件.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{NiO-LDA_U-KPOINTS}
\end{figure}

计算得到的\ch{Ni}原子的$d$电子在位占据数如图\ref{NiO-LDA_U-onsite_matrix}所示:~
\begin{figure}[h!]
\centering
...
对比考虑加$U$时(即$U=0,J=0$)时的\ch{Ni}原子的$d$电子在位占据数(如图\ref{NiO-LDA-onsite_matrix})，可以看到$U$值对$d$电子数的影响。
\begin{figure}[h!]
\centering
...
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{加$U$对反铁磁\ch{NiO}计算的\ch{Ni}的$d$电子数和局域磁矩的影响(左:;~右).}}%(与文献\cite{EPJB33-47_2003}图1对比)
...
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{加$U$对反铁磁\ch{NiO}计算的\ch{Ni}的$d$电子分态密度的影响(左:;~右).}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{NiO-LDA_U-DOS}
\end{figure}
%\subsection{光学性质计算}
%\subsection{GW计算}
