...
用\ref{Sec:FCC-Pt}的\textrm{shell}脚本\textrm{run.vasp}提交计算任务至后台运行，计算过程中可以检查输出文件\textrm{nohup.out}或\textrm{OSZICAR}的内容可以监控计算过程，图\ref{Pt_Bulk-OSZICAR}给出\textrm{nohup.out}的部分结果:
\begin{figure}[h!]
\centering
\hspace*{-25pt}
...
\textrm{VASP}程序中设定，完成最初五次电子步迭代后，才开始电荷密度的迭代，所以最初5次电子步迭代时，没有\textrm{rms(c)}输出。电子步结束时，最后一行的\textrm{F}表示电子步达到收敛的自由能，根据同一行的$\mathrm{E}_0$值(0\textrm{K}时$\sigma\rightarrow0$)，可以算出超晶胞中的单个原子\textrm{Pt}的能量为--193.8569/32=--6.058\textrm{~eV/atom}。不难看出，该值比晶格参数为$3.975\mathrm{\AA}$时计算的\textrm{FCC-Pt}的原子能量--6.056\textrm{eV/atom}仅有微小变化。

如果迭代计算过程中出错，\textrm{VASP}将会输出错误或警告提示信息，并终止程序运行。此外也可以通过\textrm{kill}命令终止程序。这里命令为:~\\
\textcolor{magenta}{\textrm{killall -9 \$VASP计算进程}}
...
各种缺陷的形成能都可以用这些简化模型来模拟。用户在构造这种包含缺陷的超晶胞时，必须切记，超晶胞必须要足够大，确保缺陷间的相互作用足够小。
...
\end{displaymath}
这就是\textrm{Pt}原子的最近邻原子间距离。\textrm{POSCAR}中共有45个\textrm{Pt}原子，按\textrm{FCC}结构的\textit{a-b-c-a-b}顺序排列。一般说，差不多当原子堆积厚度达到九层才能使薄层内两侧表层的原子间相互作用忽略不计。不过在本次练习中，我们的模型只使用五层原子，主要是考虑到节省学习过程中的时间，同时也是因为由此(薄层层数)产生误差也还在可以接受的范围内。

在\textrm{FCC-Pt}体相划出晶格矢量为
\begin{displaymath}
	(a,0,0),~\bigg(\dfraca2,\dfrac{\sqrt3a}2,0\bigg),~(0,0,c)
\end{displaymath}
的\textrm{Pt}原子密堆积单胞，这里$a=2.812\mathrm{\AA}$,~$c=\sqrt{8/3}a=1.633a$。用$3\times3\times6$~(含真空层)的单胞堆积得到模拟\textrm{Pt~(111)}表面的超晶胞，它是表面模拟中的最小重复单元。
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-init-POSCAR.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Fig:VASP_procedure}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-config.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与 文献\cite{EPJB33-47_2003}图1对比)
\label{Pt_NEB-config}
\end{figure} 

计算结果表明 收敛的基态能量是-266.87\textrm{eV/system}和\textrm{NEB}计算需要的输出文件，如\textbf{CONTCAR}和\textbf{OUTCAR}，可以确定始态能量极小值时的构型和原子位置。
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-INCAR.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Pt_NEB-INCAR}
\end{figure}
...
这里第一个数是当前假想态中受力最大的原子，当每个假想态上的原子受力低于力标准(本算例为$<0.05\mathrm{eV/\AA}$)，即达到收敛。如果起始受力太大($>10\mathrm{eV/\AA}$)，就要调整为更合适的起始结构重新来计算。计算过程中，可以用脚本\textrm{vtstscript/nebef.pl}来监控，监控信息可写到文件$\mathbf{neb}\ast\mathbf{.dat}$中。例如，经过19次和30次的离子弛豫后的数据如图\ref{Pt_NEB-VTST-1}所示:
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-VTST-1.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Pt_NEB-VTST-1}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-VTST-2.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Pt_NEB-VTST-2}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-energydiff.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
...
\subsection{催化剂}
催化剂是能够改变化学反应进程(一般是加快，也有少数是减缓)，但在反应前、后不改变组分的物质。如图\ref{Pt_NEB-reaction}所示，对于一个给定化学反应，由于催化剂的存在，降低了活化能，为反应过程提供了新的反应通道，但反应的自由能($\Delta G$)不变，也就是说，催化剂的存在并不改变化学反应的热力学过程。图示的催化剂的概念很简单，但是在实际的反应过程，因为会涉及很多结构和电子态的变化，故此非常复杂。
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_NEB-reaction.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small 催化剂的存在对反应通道的能量影响的示意图.}%(与文献\cite{EPJB33-47_2003}图1对比)
...
静态计算之后是根据当前得到的电荷密度\textbf{CHGCAR}进行非自洽计算(物性计算)，要将\textbf{INCAR}中设置参数\textit{ICHARG}=11。如图\ref{Pt_surface-DOS-INCAR}所示。
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_surface-DOS-INCAR.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_PDOS-d.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Pt_PDOS-d}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Pt_surface_PDOS.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Si_POSCAR.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Si_POSCAR}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Si_POTCAR.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Si_POTCAR}
\end{figure}
...
\begin{figure}[h!] 
\centering
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Si_Band-INCAR.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Si_Band-INCAR}
\end{figure}
...
%-------------------------------------------------------------------------------------------------------------------------
	&\textrm{Rec iprocal coordinates} &\textrm{Cartesian coordinates}
...
\begin{figure}[h!]
\centering 
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{Si_KPOINTS.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
\label{Si_KPOINTS}
\end{figure}

图\ref{Si_KPOINTS}给出的是\textrm{VASP}计算沿$\vec k$点路线$\mathrm{W}-\mathrm{L}-\Gamma-\mathrm{X}-\mathrm{W}$共计80个$\vec k$点(20点/连线$times$4组连线)的\textbf{KPOINTS}文件，对应\textrm{Brillouin}区的$\vec k$点连线如图所示\ref{VASP_train-FCC-BZ}。
\begin{figure}[h!]
\centering 
\includegraphics[height=4.5in,width=3.6in,viewport=0 0 480 630,clip]{VASP_train-FCC-BZ.png}
%\includegraphics[height=1.8in,width=4.in,viewport=30 210 570 440,clip]{PAW_projector.eps}
\caption{\small \textrm{The Flow of calculation for KS-ground state in VASP.}}%(与文献\cite{EPJB33-47_2003}图1对比)
