...
包括\textrm{POSCAR}、\textrm{INCAR}、\textrm{KPOINTS}和\textrm{POTCAR}。
...
\begin{table}[h!]
    \centering
    \begin{tabular*}{\textwidth}{|@{\extracolsep{\fill}}c|@{\extracolsep{\fill}}c|}
        \toprule
...
%	\textrm{NBLK} &\textrm{blocking for some BLAS callls}\\
        \midrule
	\textrm{SYSTEM} &计算对象的名称字符串，仅用于标识计算体系，不影响计算结果\\
%	\textrm{NWRITE} &指定更详细的输出内容到\textrm{OUTCAR}\\
	\textrm{ISTART} &指定初始波函数来源:~0-从头计算; 1-从\textrm{WAVECAR}文件读取; 2-计算体积-能量曲线时使用\\
	\textrm{ICHARG} &指定电荷密度来源:~1-从\textrm{CHARGCAR}文件读取; 2-由原子电荷密度叠加生成\\
	\textrm{ISPIN} &是否打开自旋极化:~1-不打开; 2-打开\\
	\textrm{MAGMON} & 指定每个原子上的初始磁矩\\
	\textrm{INIWAV} & 针对\textrm{ISTART = 0}时，初猜波函数的生成方案:~0-简易方案; 1-轨道电子占据数采用随机数\\
	\midrule
...
%	\textrm{NPACO, APACO} &对相关函数\\
	...
	\textrm{ISMEAR} &指定$\vec k$空间和\textrm{Fermi}面积分方案:~0-\textrm{Gaussian}积分; >0-\textrm{MP}方案; -4-四面体积分方案; -5-\textrm{Bl\"ochl}改进的四面体积分方案\\
	\textrm{SIGMA} &指定\textrm{Fermi}面积分采用的展宽系数(单位ie:~\textrm{eV}):~-4-四面体积分方法; -1-\textrm{Fermi}分布函数; 0-\textrm{Gaussian}分布函数\\
	\midrule
	\textrm{ALGO} &指定电子步矩阵对角化算法:~\textrm{Normal(Davidson)-Fast-Very\_fast(RMM-DIIS)}\\
	\textrm{IALGO} &指定离子步收敛算法:~8表示共轭梯度\textrm{(Conjugate Gradient, CG)}法，48表示\textrm{RMM-DIIS}\\
...
**SYSTEM**：。例如：`SYSTEM = Si bulk relaxation`。
