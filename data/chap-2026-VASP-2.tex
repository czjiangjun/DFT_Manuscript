...
\textrm{POTCAR}文件的主要包含以下几部分
\begin{itemize}
	\item \textrm{POTCAR}数据生成的控制参数
{\fontsize{8.2pt}{6.2pt}\selectfont{
\verbatiminput{Figures/POTCAR-1-parameter.txt} %为保险:~选用文件名绝对路径
}}
参数部分的结束，是以``PSCTR-controll parameters''为结束标记，这些参数主要是原子构型信息、超软赝势\textrm{(USPP)}\textrm{PAW}方法产生原子赝势所需的截断半经等各类参数。
...
原子赝势的局域部分以``local part''为起始标记，紧随其后的是$\vec k$空间截断值；随后为$V_{\mathrm{loc}}(G)$的1000个数值点；标记``gradient corrections used for XC''提示交换-相关泛函，代表5表示该原子势中的交换-相关部分采用\textrm{PBE}泛函形式。
...
原子电子密度的局域部分以``core charge-density (partial)''为真实电荷密度的起始标记，紧随其后为$n_{\mathrm{loc}}^{\mathrm{core}}(G)$的1000个数值点；标记``atomic pseudo charge-density''为赝电荷密度的起始，随后是$\tilde n_{\mathrm{loc}}^{\mathrm{core}}(G)$的1000个数值点。
...
投影子函数$\tilde p_l$与原子波函数相关，因此也依赖角动量和赝波函数的能量。与势函数的空间布点数(1000)不同，投影子函数在$\vec k$空间和实空间的布点数仅为100，因此$\vec k$空间的阶段值列在第一行。随后以``Non local Part''标记非局域部分的投影子函数相关的贡献。紧随第一行三个参数分别为角动量$l$及其数目和径向半径，下一行为原子超软赝势和\textrm{PAW}方法所涉及的非局域部分贡献的$D^{\mathrm{ion}}$项(共四个)。随后即为角动量$l$的投影子函数分别在倒空间和正空间的表示。
