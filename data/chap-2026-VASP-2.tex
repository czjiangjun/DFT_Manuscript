...
投影子函数$\tilde p_l$与原子波函数相关，因此也依赖角动量和赝波函数的能量。与势函数的空间布点数(1000)不同，投影子函数在$\vec k$空间和实空间的布点数近为100，因此$\vec k$空间的阶段值列在第一行。随后以``Non local Part''表示径向波函数的电荷密度差，是电荷密度差
\item 基于\textrm{PAW}的镜像波函数网格布点
\item 原子全电子势函数
\item 原子芯电荷密度
\item 原子动能密度
\item 原子的赝芯电荷密度
