...
近年来，在\textrm{NVDIA}公司的直接支持下，\textrm{VASP}软件的\textrm{GPU}版本计算性能也有了极大的提升。

为便于读者整体了解\textrm{VASP}的软件框架和底层逻辑，以下对该软件所涉及的密度泛函和第一原理分子动力学相关的基础理论作扼要介绍，

\section{\rm{VASP}软件的基础理论}
\subsection{密度泛函{理论}基础} \label{chap:dft}
密度泛函理论\upcite{Parr-Yang,CR91-651_1991}%最早})
{的源头}可以追溯到二十世纪二十年代的Thomas-Fermi%理论})
{模型}\upcite{PCPS23-542_1927,ZP48-73_1928}和五十年代Slater提出的X\,$\alpha$方法\upcite{PR81-385_1951}。与传统的量子%化})
{力}学方法不同，密度泛函理论的%最基本的变量是体系单粒子})
{基本变量是体系的电子}密度，通过体系的%单})
%粒})
{电}子密度而非波函数来确定体系基态和性质，如总能量等。直到六十年代，Hohenberg和Kohn\upcite{PR136-B864_1964}严格%地})
证明了多%粒})
{电}子%量子力学})
体系的%基态
性质%可以})
...
{电}子密度%使得})
{对应于}基态总能量%变分取极小})
{泛函的最小极小值}，为密度泛函理论奠定了坚实的理论基础。遗憾的是，%因为})
{迄今无法知道}体系的基态总能量%和})
{作为}基态的%单})
%粒})
{电}子密度%之间的具体对应关系并不清楚，特别是体系的动能与基态单粒子的对应关系对于精确计算体系基态总能量有比较重要的影响})
{的泛函的显式表达}。Kohn和Sham\upcite{PR140-A1133_1965}%采用了与相互作用体系有相同单粒子密度的无相互作用体系的动能近似来近似计算相互作用体系的动在在能，把剩余部分都放在交换-相关能项中予以考虑。本质上，这个方法把单粒子波函数又引入了密度泛函理论，但是基本解决了动能计算的问题。在他们的工作基础上，各种近似的交换-相关能泛函形式\upcite{CJP58-1200_1980,PRB45-13244_1992,PRA38-3098_1988,PRB33-8822_1986,PRB37-785_1988}被提出来并})
...
Hohenberg-Kohn定理\upcite{PR136-B864_1964}在内容上可分为两部分。第一部分指出，多%粒})
{电}子体系的外势由其%基态性质与
基态%单})
%粒})
