...
%### 3.1 DOSCAR概述
%
态密度\textrm{(Density of States, DOS)}是计算电子结构最核心的工具之一，通过DOSCAR可以绘制出总态密度（TDOS）和分波态密度（PDOS）图。
DOSCAR**文件包含了体系的**态密度**（Density of States, DOS）和**积分态密度**（Integrated DOS）。其单位分别为**状态数/eV**和**状态数**。
%
