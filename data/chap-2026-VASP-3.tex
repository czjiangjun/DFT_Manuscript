...
\subsection{\textcolor{red}{\rm{VASP}的并行优势:~轮询调度算法}}
\subsection{\textcolor{red}{\rm{VASP}的精度保证:~双网格技术}}

\section{\rm{VASP}的输出文件}
对用户而言，准备好输入文件并成功提交服务器，只是完成\textrm{VASP}计算的第一步。计算完成后，\textrm{VASP}将产生一系列输出文件。如何从这些输出文件中快速、准确地提取体系能量、结构、电子态密度、能带、电荷密度等信息，是每一位\textrm{VASP}用户必须掌握的基本技能。

\textrm{VASP}的输出文件体系庞大而完整，主要包括:~输出文件\textrm{OUTCAR}，其中包含计算过程的完整日志；结构文件\textrm{CONTCAR}，与\textrm{POSCAR}不同，\textrm{CONTCAR}记录了每次迭代后更新后的晶体结构；态密度文件\textrm{DOSCAR}，能量本征值文件\textrm{EIGENVAL}，电荷密度文件\textrm{CHGCAR}，体系本征态波函数文件\textrm{WAVECAR}，投影态波函数文件\textrm{PROCAR}，迭代计算过程中能量收敛纪录文件\textrm{OSZICAR}，局域势函数文件\textrm{LOCPOT}，电子局域函数文件\textrm{ELFCAR}，\textrm{Brillouin}区不可约$\vec k$点文件\textrm{IBZKPT}，离子轨迹文件\textrm{XDATCAR}等。此外，\textrm{VASP}还支持输出\textrm{XML}格式的输出文件\textrm{vasprun.xml}%和\textrm{HDF5}格式的输出文件\textrm{vaspout.h5}(需要有相关库函数支持)，
...
\subsection{常用输出文件:~\rm{CONTCAR}}\label{subsect:CONTCAR}
...
\subsection{常用输出文件:~\rm{OSZICAR}}\label{subsectr:OSZICAR}
...
此处第一项\textrm{T}为当前离子步的温度:~\textrm{E}是总的自由能(包括离子动能和\textrm{Nos\'e}恒温方案的能量的贡献)；\textrm{F}与$\mathrm{E}_0$与前面一样；\textrm{EK}是体系的动能；\textrm{SP}是来自\textrm{Nos\'e}恒温方案的势能部分的贡献；\textrm{SK}则是来自\textrm{Nos\'e}恒温方案的动能部分贡献。
...
%
\subsection{常用输出文件:~\rm{DOSCAR}}\label{subsectr:DOSCAR}
%## 三、DOSCAR：态密度文件
%
%### 3.1 DOSCAR概述
%
...
%
%
\subsection{常用输出文件:~\rm{EIGENVAL}}\label{subsectr:EIGENVAL}
%## 四、EIGENVAL：本征值文件
%
%### 4.1 EIGENVAL概述
%
...
%
%
\subsection{常用输出文件:~\rm{CHGCAR}}\label{subsectr:CHGCAR}
%## 五、CHGCAR：电荷密度文件
...
\subsection{常用输出文件:~\rm{WAVECAR}}\label{subsectr:WAVECAR}
%## 六、WAVECAR：波函数文件
...
%## 十、ELFCAR：电子局域函数文件
如果需要分析``原子之间成键''或考虑``电子局域化程度''等与周期体系中电子局域分布有关的问题，就需要\textrm{ELFCAR}文件中的数据(设置参数\texttt{LELF}=\textrm{.TRUE.})，\textrm{ELFCAR}的文件格式与\textrm{CHG}文件类似，因此其存储的电子局域函数\textrm{Electron Localization Function, ELF}可直接用\textrm{VESTA}等工具直接可视化展示。计算\textrm{ELF}须注意的是:
\begin{itemize}
	\item 为避免圆整误差，建议设置参数\texttt{PREC}=\textrm{High}，采用高精度的计算；
...
在结构弛豫和\textrm{MD}计算中，每个离子步的原子径迹(\textrm{trajectory})由\textrm{XDATCAR}文件记录。参数\texttt{NBLOCK}=\textrm{N}控制每经过\textrm{N}个离子步，在\textrm{XDATCAR}中记录一次当前的原子构型。
{\fontsize{8.2pt}{6.2pt}\selectfont{
	\lstinputlisting[linerange=1-23]{Figures/XDATCAR} %为保险:~选用文件名绝对路径
}}
这些数据可借助可视化工具如\textrm{OVITO}等，展示结构弛豫或\textrm{MD}过程中各原子的行经轨迹，也可以用于提取特定离子步的原子构型，为后续分析提供依据。
%### 12.1 XDATCAR概述
%
...
%
%
\textrm{DFT}计算的核心数据无非是体系总能量(总自由能)、电子结构(含态密度和能带)、电荷密度与\textrm{Kohn-Sham}本征态波函数。对应到\textrm{VASP}计算就是\textrm{OSZICAR}、\textrm{DOSCAR}、\textrm{EIGENVAL}、\textrm{CHGCAR}和\textrm{WAVECAR}。此外，上面还介绍了一些常用于数据分析和可视化的输出文件。还有一些更少用到的文件，这里只简要提一下:
\begin{itemize}
