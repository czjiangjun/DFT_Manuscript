...
对用户而言，准备好输入文件并成功提交服务器，只是\textrm{VASP}计算万里长征的第一步。计算完成后，所有结果都包含在\textrm{VASP}产生的一系列输出文件中，如何从这些输出文件中快速、准确地提取体系能量、结构、电子态密度、能带、电荷密度等信息，是每一位\textrm{VASP}用户必须掌握的核心技能。
...
%### 1.2 OUTCAR的文件结构
一般地，\textrm{OUTCAR}文件可总体划分为几个主要部分：
\begin{itemize}
	\item \textrm{VASP}的版本和并行信息；
...
%### 1.3 OUTCAR中的关键信息解析
%#### （一）输入参数汇总
%\textrm{OUTCAR}的开头部分会\textrm{INCAR}中的所有参数逐行打印出来，方便用户确认计算设置是否正确。这部分还包括从\textrm{POTCAR}中读取的赝势信息、从\textrm{POSCAR}中读取的结构信息以及从\textrm{KPOINTS}中读取的k点设置
