...
目前广泛应 用于多种有机高分子材料的研发应用，如筛选有机太阳能电池、有机半导体、燃料电池中的高分子膜材料等。\textrm{CEP}的计算方案中，结合了传统材料模拟方案和现代药物研发模式，通过集成\textrm{MOPAC}\cite{JCAMD4-1-1990}， \textrm{Q-Chem}\cite{PCCP8-3172_2006}系统计算并建立了覆盖$\sim1,000,000$种电子结构的候选光电材料数据库(\textrm{MySOL}，\textrm{Django}形式)；在此基础上，\taxtrm{CEP}希望借助化学信息学和数据挖掘技术，更高效、快速地筛选合适的光电材料。化学信息学辅助的机器学习是\textrm{CEP}的显著技术特色。 但\textrm{CEP}的大部分功能模块都只是部分完成或还在研发中，其代码也未开源，因此\textrm{CEP}的完善程度不如其他几类软件平台，目前主要在完成数据库的积累。 
