...
\begin{equation}
  ...
  \label{eq:bloch}
\end{equation}
$k$是与平移对称性相应的量子数，因此理想晶体的电子波函数除了与势函数具有相同的平移周期性，还受到平面波的相位控制。密度泛函理论发展起来之后，为包括能带在内的电子结构计算提供了坚实的基础，可以通过选定交换-相关泛函(主要是LDA或GGA)，自洽迭代求解Kohn-Sham方程直接得到电子能带。在具体应用DFT求解能带时，还需要考虑各种近似，不同近似方法的差别在于单电子有效势和波函数形式的选取两方面。
